...
class circle_manageControl extends SystemControl{
	...
	public function indexOP(){
        ...
        //$condition=array(),$field='',$order = 'theme_addtime DESC'
	    $result=$circle_theme->getmemberAlltheme($condition=array(),'*','theme_addtime DESC');

	    Tpl::output('show_page', $circle_theme->showpage());
	    Tpl::output('cirlce_list', $result);
	    Tpl::showpage('circle_list.index');
	}
	
	/*后台管理员 推荐的帖子 xuping*/
	public function re_theme_recommedOP(){
	    $circle_theme = Model('circle_theme');
	    $theme_id=intval($_GET['theme_id']);
	    $condition['theme_id']=$theme_id;
	    $result=$circle_theme->getmemberAlltheme($condition,'*','theme_addtime DESC');
        if($result[0]['is_closed']==0){
           $where['theme_id']=$theme_id;
           $data['is_recommend']=1;
           $circle_theme->updateTheme($where,$data);
           $result=$circle_theme->getmemberAlltheme($condition=array(),'*','theme_addtime DESC');
           Tpl::output('show_page', $circle_theme->showpage());
           Tpl::output('cirlce_list', $result);
           Tpl::showpage('circle_list.index');
		...
		public function theme_recommedOP(){
	    $circle_theme = Model('circle_theme');
	    $theme_id=intval($_GET['theme_id']);
	    $condition['theme_id']=$theme_id;
	    $result=$circle_theme->getmemberAlltheme($condition,'*','theme_addtime DESC');
	...
	public function theme_delOP(){
        ...
        $circle_theme->delTheme($condition);
        $result=$circle_theme->getmemberAlltheme($condition=array(),'*','theme_addtime DESC');
    ...
    public function theme_detailOP(){
        $circle_theme=Model('circle_theme');
        $result=$circle_theme->find(intval($_GET['theme_id']));
        $circle_report=Model('circle_report')->where(array('theme_id'=>intval($_GET['theme_id'])))->select();
        if($circle_report){
            $result['report_info']=$circle_report;
        }
        
        Tpl::output('cirlce', $result);
        Tpl::showpage('circle_detail.index');
    }
	/*举报帖子 后端屏蔽帖子 xuping*/
    public function theme_nodelOP(){
        $circle_theme = Model('circle_theme');
        $theme_id=intval($_GET['theme_id']);
        $condition['theme_id']=$theme_id;
        $result=$circle_theme->getmemberAlltheme($condition,'*','theme_addtime DESC');
...
//						@unlink(BASE_UPLOAD_PATH.DS.ATTACH_CIRCLE.'/group/'.$circle_list['circle_id'].'.jpg');
                        ...
                        $QNupload->delFileQny(ATTACH_CIRCLE.'/group/'.$circle_list['circle_id'].'.jpg');
					}
				}
				// 删除附件
...
//            @unlink(BASE_UPLOAD_PATH.DS.ATTACH_CIRCLE.'/group/'.$circle_info['circle_id'].'.jpg');
            $QNupload=new uploadFile();
            $QNupload->delFileQny(ATTACH_CIRCLE.'/group/'.$circle_info['circle_id'].'.jpg');
        }
