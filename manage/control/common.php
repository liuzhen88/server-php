...
class commonControl extends SystemControl{
	...
	public function pic_uploadOp(){
		if (chksubmit()){
			...
			if (!empty($_FILES['_pic']['tmp_name'])){
				$result = $upload->upfile_old('_pic');
				if ($result){
                    exit(json_encode(array('status'=>1,'url'=>C('upload_site_url_old').'/'.$_GET['uploadpath'].'/'.$upload->thumb_image)));
				}else {
	...
	public function pic_cutOp(){
		...
		if (chksubmit()){
			...
			$h = $_POST["h"];
			$scale = $thumb_width/$w;
			$src = str_ireplace(C('upload_site_url_old'),BASE_UPLOAD_PATH,$_POST['url']);
...
//			}
            ...
            //将图片转存到七牛云
            $upload = new UploadFile();
            $qny_path=str_replace(BASE_UPLOAD_PATH,'',$save_file2);
            $qny_path=trim($qny_path,DS);
            $upload->delFileQny($qny_path);
            $upload->uploadFileQny($save_file2,$qny_path);
			@unlink($src);
	...
	public function mapOp(){
		Tpl::showpage('baidu.map');
	}

	public function nick_name_file_to_array_fileOp(){
		$res=file_get_contents(BASE_DATA_PATH.'/resource/nickname.txt');
		$res_array=explode(PHP_EOL,$res);
		$res_string=var_export ($res_array,true);
		$res_string="<?php defined('emall') or exit('Access Invalid!'); return ".$res_string.";";
		file_put_contents(BASE_DATA_PATH.'/resource/nickname.php',$res_string);
	}
}
