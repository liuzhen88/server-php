...
//	}
	...
	public function manage_saveOp() {
        ...
        if(!empty($_FILES['microshop_logo']['name'])) {
            ...
            $old_image[] = ATTACH_MICROSHOP.DS.C('microshop_logo');
        }
        if(!empty($_FILES['microshop_header_pic']['name'])) {
            ...
            $old_image[] = ATTACH_MICROSHOP.DS.C('microshop_header_pic');
        }
    ...
    private function class_save($type) {
        ...
        if(!empty($_FILES['class_image']['name'])) {
            ...
            if(!empty($_POST['old_class_image'])) {
                $upload->delFileQny(ATTACH_MICROSHOP.DS.$_POST['old_class_image']);
        ...
        if(!empty($list)) {
            foreach ($list as $value) {
                if(!empty($value['class_image'])) {
                    ...
                    $QNupload->delFileQny(ATTACH_MICROSHOP.DS.$value['class_image']);
                    //删除老图片
        ...
        if(!empty($list)) {
            foreach ($list as $personal_info) {
                ...
                foreach ($image_array as $value) {
                    //删除原始图片
                    $QNupload=new uploadFile();
                    $QNupload->delFileQny(ATTACH_MICROSHOP.DS.$personal_info['commend_member_id'].DS.$value);
...
//                    }
                    ...
                    $ext = explode('.', $value);
                    $ext = $ext[count($ext) - 1];
                    $QNupload->delFileQny(ATTACH_MICROSHOP.DS.$personal_info['commend_member_id'].DS.$value.'_list.'.$ext);
//                    $image_name = BASE_UPLOAD_PATH.DS.ATTACH_MICROSHOP.DS.$personal_info['commend_member_id'].DS.$value.'_list.'.$ext;
//                    if(is_file($image_name)) {
//                        unlink($image_name);
//                    }
                    $QNupload->delFileQny(ATTACH_MICROSHOP.DS.$personal_info['commend_member_id'].DS.$value.'_tiny.'.$ext);
    ...
    public function adv_saveOp() {
        ...
        if(!empty($_FILES['adv_image']['name'])) {
            ...
            if(!empty($_POST['old_adv_image'])) {
                $QNupload=new uploadFile();
                $QNupload->delFileQny(ATTACH_MICROSHOP.DS.'adv'.DS.$_POST['old_adv_image']);
    ...
    public function adv_dropOp() {
        ...
        if(!empty($list)) {
            foreach ($list as $adv_info) {
                //删除原始图片
                $QNupload=new uploadFile();
                $QNupload->delFileQny(ATTACH_MICROSHOP.DS.'adv'.DS.$adv_info['adv_image']);
