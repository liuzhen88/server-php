...
class predepositControl extends SystemControl{
	...
	public function predepositOp(){
		...
		$recharge_list = $model_pd->getPdRechargeList($condition,self::ONE_PAGE_SIZE,'*','pdr_id desc');
		//信息输出
	...
	public function pd_log_listOp(){
		...
		$model_pd = Model('predeposit');
		$list_log = $model_pd->getPdLogList($condition,self::ONE_PAGE_SIZE,'*','lg_id desc');
		Tpl::output('show_page',$model_pd->showpage());
	...
	public function pd_cash_listOp(){
		...
		$model_pd = Model('predeposit');
		if ($start_unixtime || $end_unixtime) {
			$condition['pd_cash.pdc_add_time'] = array('time',array($start_unixtime,$end_unixtime));
		}
		if (!empty($_GET['mname'])){
			$condition['pd_cash.pdc_member_name'] = $_GET['mname'];
		}
		if (!empty($_GET['pdc_bank_user'])){
			$condition['pd_cash.pdc_bank_user'] = $_GET['pdc_bank_user'];
		}
		if ($_GET['paystate_search'] != ''){
			$condition['pd_cash.pdc_payment_state'] = $_GET['paystate_search'];
		}
		$cash_list = $model_pd->getPdCashList($condition,self::ONE_PAGE_SIZE,'*','pdc_payment_state asc,pdc_id desc','',$_GET['role_search']);
		$cash_sum = $model_pd->getPdCashSum($condition,$_GET['role_search']);
		Tpl::output('total',$cash_sum['sum']);
	...
	public function pd_cash_payOp(){
        ...
        $update = array();
        $admininfo = $this->getAdminInfo();
        $update['pdc_payment_state'] = $_REQUEST['cash_state'];
        $update['pdc_payment_admin'] = $admininfo['name'];
		if($_REQUEST['cash_state']==1||$_REQUEST['cash_state']==3) {
			if($info['pdc_payment_state']==0) {
				$update['pdc_deal_time'] = TIMESTAMP;
			}
			$update['pdc_payment_time'] = TIMESTAMP;
		}
		elseif($_REQUEST['cash_state']==2) {
			$update['pdc_deal_time'] = TIMESTAMP;
		}
		if(isset($_REQUEST['pdc_message'])&&!empty($_REQUEST['pdc_message'])) {
			$update['pdc_message'] = $_REQUEST['pdc_message'];
		}
        ...
        try {
            ...
            $data['order_sn'] = $info['pdc_sn'];
            $data['admin_name'] = $admininfo['name'];
			if($_REQUEST['cash_state']==1) {
				$model_pd->changePd('cash_pay',$data);
			}
  			elseif($_REQUEST['cash_state']==3) {
				$model_pd->changePd('cash_del',$data);
			}
            $model_pd->commit();
            $this->log($log_msg,1);
	...
	public function export_cash_step1Op(){
		...
		if (!is_numeric($_GET['curpage']) && !is_numeric($_GET['cursection'])){
			$count = $model_pd->getPdCashCount($condition,$_GET['role_search']);
			$array = array();
			if ($count > self::EXPORT_SIZE ){	//显示下载链接
				$page = ceil($count/self::EXPORT_SIZE);
				for ($i=1;$i<=$page;$i++){
				...
				$data = $model_pd->getPdCashList($condition,'','*','pdc_payment_state asc,pdc_id asc',self::EXPORT_SIZE,$_GET['role_search']);
			...
			$limit1 = ($_GET['cursection']-1) * self::EXPORT_SIZE;
			$limit2 = self::EXPORT_SIZE;
			$data = $model_pd->getPdCashList($condition,'','*','pdc_payment_state asc,pdc_id asc',"{$limit1},{$limit2}",$_GET['role_search']);
			$cashpaystate = array(0=>'未支付',1=>'已支付');
			foreach ($data as $k=>$v) {
				$data[$k]['pdc_payment_state'] = $cashpaystate[$v['pdc_payment_state']];
			}
