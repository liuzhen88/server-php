...
class memberControl extends SystemControl {
    ...
    public function memberOp() {
        ...
        for($i=0;$i<count($member_list);$i++){
            $where['member_id'] = $member_list[$i]['member_id'];
            $member_list[$i]['role'] = "普通用户";
            //判断是否是代理商
            $agent_info = Model('agent')->getAgentInfo($where);
            if(!empty($agent_info)){
                $member_list[$i]['role'] = "代理商";
                continue;
            }
            //判断是否是商户或商户员工
            $sell_info = Model('seller')->getSellerInfo($where);
            if(!empty($sell_info)){
                if($sell_info['is_admin']==1){
                    $member_list[$i]['role'] = "商户";
                }
                else {
                    $member_list[$i]['role'] = "商户员工";
                }
                continue;
            }
            //判断是否是代理商员工
            $agent_manage = Model('agent_manager')->getAgentManagerInfo($where);
            if(!empty($agent_manage)){
                $member_list[$i]['role'] = "代理商员工";
            }
        }
    ...
    public function member_editOp() {
        ...
        if (chksubmit()) {
             ...
             * 验证
             */
            if(empty($_REQUEST['member_invitation'])) {
                showMessage('邀请码不能为空');
            }
            ...
            if ($error != '') {
                ...
                if (!empty($_POST['member_passwd'])) {
                    ...
                    $update_array['member_salt']   = $salt;
                    $update_array['member_passwd'] = md6($_POST['member_passwd'], $salt);
                }
                ...
                $update_array['invitation'] = $_POST['member_invitation'];
                if (!empty($_POST['member_avatar'])) {
                    $update_array['member_avatar'] = $_POST['member_avatar'];
                }
                $member_info = $model_member->getMemberInfo(array('member_id'=>intval($_POST['member_id'])));
                $message = '';
                if($member_info['invitation']!=$_POST['member_invitation']) {
                    $member_result = $model_member->getMemberInfo(array('invitation'=>$_POST['member_invitation']));
                    if(!empty($member_result)) {
                        showMessage('邀请码已存在，请更换');
                    }
                    $message = ' 邀请码从'.$member_info['invitation'].'改为'.$_POST['member_invitation'];
                }
                ...
                if ($result) {
                    ...
                    $this->log(L('nc_edit,member_index_name') . '[ID:' . $_POST['member_id'] . ']'.$message, 1);
