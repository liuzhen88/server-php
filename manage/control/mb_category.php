...
class mb_categoryControl extends SystemControl{
	...
	public function mb_category_listOp(){ 
		 ...
		 * 商品分类
		 */
		$goods_class = Model('goods_class')->getGoodsClassForCacheModel('all');
	...
	public function mb_category_addOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			$return = Model('goods_class')->getGoodsClassLineForTag($_POST['class_id']);


			$insert_array = array();
			$insert_array['gc_id'] = trim($_POST['class_id']);
			$insert_array['gc_thumb'] = trim($_POST['link_pic']);
			$insert_array['gc_tag_name'] = $return['gc_tag_name'];
