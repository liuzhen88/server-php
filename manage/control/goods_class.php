...
class goods_classControl extends SystemControl{
	...
	public function goods_class_addOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				$insert_array['commis_rate']    = floatval($_POST['commis_rate']);
				$insert_array['distribution_rate']    = floatval($_POST['distribution_rate']);
				$insert_array['gc_sort']		= intval($_POST['gc_sort']);
                $insert_array['gc_virtual']     = intval($_POST['gc_virtual']);
				$insert_array['class_type']    = intval($_POST['class_type']);
		...
		/*从管理页面进去， 实则添加的是二级导航, 页面中保留类别和一级分类*/
		if(!empty($_GET['gc_parent_id']) && isset($_GET['class_type'])){
			Tpl::output('tag',$_GET['gc_parent_id']);
			$res=Model()->table('goods_class')->where(array('gc_id'=>intval($_GET['gc_parent_id'])))->field('gc_name')->find();
			Tpl::output('gc_class_name',$res['gc_name']);
			Tpl::output('fag',$_GET['class_type']);
		}
	...
	public function goods_class_editOp(){
		...
		if (chksubmit()){
			...
			$update_array['class_type']     = intval($_POST['class_type']);
			$update_array['distribution_rate']     = floatval($_POST['distribution_rate']);
            ...
            if ($_POST['d_distribution_rate'] == '1' || $_POST['t_commis_rate'] == '1' || $_POST['t_associated'] == '1' || $_POST['t_gc_virtual'] == '1'|| $_POST['t_class_type'] == '1') {
            ...
            //平台抽佣
            if ($_POST['d_distribution_rate'] == '1' && !empty($gc_ids)){
                $model_class->editGoodsClass(array('distribution_rate'=>$update_array['distribution_rate']),array('gc_id'=>array('in',$gc_ids)));
            }
			...
			if ($_POST['t_class_type'] == '1' && !empty($gc_ids)){
	            $model_class->editGoodsClass(array('class_type'=>$update_array['class_type']),array('gc_id'=>array('in',$gc_ids)));
			}
		...
		$class_array = $model_class->getGoodsClassInfoById(intval($_GET['gc_id']), 'all');
		...
		Tpl::output('gc_list', $gc_list);
	    $pic_name = UPLOAD_SITE_URL.'/'.ATTACH_COMMON.'/category-pic-'.$class_array['gc_id'].'.jpg';
			...
			if (!empty($_FILES['csv']) && !empty($_FILES['csv']['name']) && $file_type == 'csv'){
				...
				while (!feof($fp)) {
					...
					if (!empty($data)){
                        ...
                        $insert_array['class_type']=$class_type;
			...
			foreach ($goods_class as $val) {
				...
				if($val['gc_parent_id'] == 0) {
					...
					if(isset($goods_class_array[$val['gc_parent_id']])){
						...
						$goods_class_array[$val['gc_parent_id']]['sub_class'][$val['gc_id']]['class_type']			= $val['class_type'];
					}else{
						foreach ($goods_class_array as $v){
							...
							if(isset($v['sub_class'][$val['gc_parent_id']])){
								...
								$goods_class_array[$v['sub_class'][$val['gc_parent_id']]['gc_parent_id']]['sub_class'][$val['gc_parent_id']]['sub_class'][$val['gc_id']]['class_type']	= $val['class_type'];
							}
						}
					}
				}
	...
	public function ajaxOp(){
		switch ($_GET['branch']){
			...
			case 'check_class_name':
				...
				/*xuping  添加、编辑分类时分类名称在同类别下不可重复，在不同类别下可以重复  添加*/
				$condition['class_type']=intval($_GET['class_type']);
