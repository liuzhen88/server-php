...
class specControl extends SystemControl {
	...
	public function specOp(){
		...
		$spec_list	= $model_spec->specList(array('order'=>'sp_sort asc,sp_id asc'), $page);
	...
	public function spec_addOp(){
		...
		// 一级商品分类
		$gc_list = Model('goods_class')->getGoodsClassListByParentId(0,0);//只有商城商品有规格，不需要本土分类
...
//			case 'name':
	...
	public function export_step1Op(){
		...
		if (!is_numeric($_GET['curpage']) && !is_numeric($_GET['cursection'])){
			$count = $model_spec->getSpecTotalNum();
			$array = array();
			...
			$spec_list	= $model_spec->specList(array('order'=>'sp_sort asc,sp_id asc'), $page,'',"{$limit1},{$limit2}");
			$this->createExcel($spec_list);
		}else{	//分页下载
