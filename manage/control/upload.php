...
class uploadControl extends SystemControl{
	...
	public function default_thumbOp(){
		...
		if (chksubmit()){
			...
			//默认跑腿邦店铺头像
			if (!empty($_FILES['default_league_store_avatar']['tmp_name'])){
				$upload->set('file_name', '');
				$upload->set('thumb_width',	0);
				$upload->set('thumb_height',0);
				$upload->set('thumb_ext',	false);
				$result = $upload->upfile('default_league_store_avatar');
				if ($result){
					$_POST['default_league_store_avatar'] = $upload->file_name;
				}else {
					showMessage($upload->error,'','','error');
				}
			}
			...
			if (!empty($_POST['default_league_store_avatar'])){
				$update_array['default_league_store_avatar'] = $_POST['default_league_store_avatar'];
			}
...
//					@unlink(BASE_UPLOAD_PATH.DS.ATTACH_COMMON.DS.$list_setting['default_goods_image']);
                    ...
                    $QNupload->delFileQny(ATTACH_COMMON.DS.$list_setting['default_goods_image']);
...
//					}
				...
				if (!empty($list_setting['default_store_logo']) && !empty($_POST['default_store_logo'])){
                    $QNupload=new uploadFile();
                    $QNupload->delFileQny(ATTACH_COMMON.DS.$list_setting['default_store_logo']);
//					@unlink(BASE_UPLOAD_PATH.DS.ATTACH_COMMON.DS.$list_setting['default_store_logo']);
				...
				if (!empty($list_setting['default_user_portrait']) && !empty($_POST['default_user_portrait'])){
                    $QNupload=new uploadFile();
                    $QNupload->delFileQny(ATTACH_COMMON.DS.$list_setting['default_user_portrait']);
...
//					@unlink(BASE_UPLOAD_PATH.DS.ATTACH_COMMON.DS.str_ireplace(',', '_small.', $list_setting['default_user_portrait']));
				...
				if (!empty($list_setting['default_league_store_avatar']) && !empty($_POST['default_league_store_avatar'])){
					$QNupload=new uploadFile();
					$QNupload->delFileQny(ATTACH_COMMON.DS.$list_setting['default_league_store_avatar']);
					//					@unlink(BASE_UPLOAD_PATH.DS.ATTACH_COMMON.DS.$list_setting['default_user_portrait']);
					//					@unlink(BASE_UPLOAD_PATH.DS.ATTACH_COMMON.DS.str_ireplace(',', '_small.', $list_setting['default_user_portrait']));
				}
