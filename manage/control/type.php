...
class typeControl extends SystemControl {
	...
	public function type_editOp(){
		...
		// 一级分类列表
		$gc_list = Model('goods_class')->getGoodsClassListByParentId(0,0);
		Tpl::output('gc_list', $gc_list);
...
//			case 'name':
	...
	public function export_step1Op(){
		...
		if (!is_numeric($_GET['curpage']) && !is_numeric($_GET['cursection'])){
			$count = $model_type->getTypeTotalNum();
			$array = array();
			...
			$type_list	= $model_type->typeList(array('order'=>'type_sort asc'),$page,'',"{$limit1},{$limit2}");
			$this->createExcel($type_list);
		}else{	//分页下载
