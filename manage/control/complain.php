...
class complainControl extends SystemControl {
    ...
    const LEAGUE_COMPLAIN_NEW = 10;
    protected static $league = array(
     ...
     *
     */
    public function leagueNewListOp()
    {
       $this->_getLeagueList(self::$league['complain_state']['new'],'leagueNewList');
     ...
     * 待仲裁列表
     */
    public function leagueWaitArbitrateOp()
    {
        $where = array(
            'where'=>array(
                'complain_handle_manage_id' => $this->admin_info['id']
            )
        );
        $this->_getLeagueList(self::$league['complain_state']['wait_arbitrate'],'leagueWaitArbitrate',$where);

    }

    public function leagueFinishListOp()
    {
        $this->_getLeagueList(self::$league['complain_state']['finish'],'leagueFinishList');

    }

    /**
     * @param int $type 状态码
     * @param string $op  当前操作链接
     */
    private function _getLeagueList($type, $op, $extend = array()){
        $this->_topNav($op);
        Tpl::output('op',$op);
        $leagueComplain = Model('LeagueComplain');
        $page = new Page();
        $page->setEachNum(10) ;
        $page->setStyle('admin') ;
        $condition = array();
        $condition['accuser_name'] = trim($_POST['input_complain_accuser']);
        $condition['league_store_name'] = trim($_POST['input_complain_accused']);
        $condition['complain_datetime_start'] = strtotime($_GET['input_complain_datetime_start']);
        $condition['complain_datetime_end'] = strtotime($_GET['input_complain_datetime_end']);
        if(isset($extend['where'])){
            $condition = array_merge($condition,$extend['where']);
        }
        if($op === 'finish') {
        ...
        $condition['complain_state'] = $type;
        $listData = $leagueComplain->getList($condition,$page);
        Tpl::output('op',$op);

        $this->_listPage($listData,$page->show());
    }


    private function _listPage($listData ,$pageShow){
        Tpl::output('list', $listData) ;
        Tpl::output('show_page',$pageShow) ;
        Tpl::showpage('league_complain.list') ;
    }


    /**
     * 跑腿邦页面导航
     * @param string $menuKey 判断是否选中
     */
    private function _topNav($menuKey){
        $menu_array = array(
            'leagueNewList'=>array('menu_type'=>'link','menu_name'=>'新投诉','menu_url'=>'index.php?act=complain&op=leagueNewList'),
            'leagueWaitArbitrate'=>array('menu_type'=>'link','menu_name'=>'待仲裁','menu_url'=>'index.php?act=complain&op=leagueWaitArbitrate'),
            'leagueFinishList'=>array('menu_type'=>'link','menu_name'=>'已关闭','menu_url'=>'index.php?act=complain&op=leagueFinishList'),
            'complain_progress'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_progress'),'menu_url'=>'###'),
        );
        if(isset($menu_array['complain_progress']) && $menuKey !== 'complain_progress') {
            unset($menu_array['complain_progress']);
        }

        $menu_array[$menuKey]['menu_type'] = 'text';
        Tpl::output('menu',$menu_array);
    }
     ...
     * 投诉反馈处理详情
     */
    public function leagueComplainProgressOp()
    {
        $complain_id = intval($_GET['complain_id']);
        //获取投诉详细信息
        $model_lc = Model('LeagueComplain');
        $complain_info = $model_lc->getLeagueComplainInfo($complain_id);
        if(empty($complain_info)) {
            showMessage(Language::get('param_error'));
        }
        //更改投诉反馈信息
        if($complain_info['complain_state']==self::$league['complain_state']['new']){
            $condition['id'] =  $complain_id;
            $update = array(
                'complain_state' =>self::$league['complain_state']['wait_arbitrate'],
                'complain_handle_datetime' =>time(),
                'complain_handle_manage_id' =>$this->admin_info['id'],
            );
            $model_lc->save($update,$condition);
        }
        $this->_topNav('complain_progress');

        $model_order =  Model('order');
        $order_info = $model_order->getOrderInfo(array('order_id'=>$complain_info['order_id']),array(),'add_time');
        $complain_info['order_add_time'] =  $order_info['add_time'];
        Tpl::output('complain_info',$complain_info);
        Tpl::output('league_text',self::$text);
        Tpl::output('league_num',self::$league);
        Tpl::showpage('league_complain.info');
    }
     ...
     * 更改处理状态和备注
     */
    public function leagueSaveOp()
    {
        $update['final_handle_message'] =  trim($_POST['final_handle_message']);
        $update['handle_type'] =  (int) filter_input(INPUT_POST,'handle_type');
        $condition['id'] =  (int) filter_input(INPUT_POST,'id');

        if(empty($update['final_handle_message'])){
            showMessage('处理信息不能为空');
        }
        elseif (mb_strlen($update['final_handle_message'])>255) {
            showMessage('处理信息超过255个字符');
        }
        $model_lc = Model('LeagueComplain');

        $r = $model_lc->save($update,$condition);
        if($r){
            showMessage('修改成功');
        }else{
            showMessage('修改失败');
