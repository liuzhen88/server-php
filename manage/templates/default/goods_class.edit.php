  ...
  <form id="goods_class_form" name="goodsClassForm" enctype="multipart/form-data" method="post">
    ...
    <table class="table tb-type2">
      <tbody>
        ...
        <tr>
            <td colspan="2" class="required"><label class="validation">平台抽佣:</label>
            <span>
            <label for="d_distribution_rate">
                <input id="d_distribution_rate" class="checkbox" type="checkbox" checked="checked" value="1" name="d_distribution_rate">
                关联到子分类</label>
            </span></td>
        </tr>

        <tr class="noborder">
            <td class="vatop rowform"><input id="d_commis_rate" class="w60" type="text" value="<?php echo $output['class_array']['distribution_rate'];?>" name="distribution_rate">
                % </td>
            <td class="vatop tips">必须为0-100的数</td>
        </tr>
		<tr>
          <td colspan="2" class="required"><label class="validation">商品类别:</label>
            <span>
            <label for="t_commis_rate">
              <input id="t_class_type" class="checkbox" type="checkbox" checked="checked" value="1" name="t_class_type">
              关联到子分类</label>
            </span></td>
        </tr>
		<tr class="noborder">
          <td class="vatop rowform"><select name="class_type" disabled>
              <option value="0" <?php if($output['class_array']['class_type'] == 0){?>selected="selected"<?php }?>>商城</option>
              <option value="1" <?php if($output['class_array']['class_type'] == 1){?>selected="selected"<?php }?>>本土</option>
              <option value="3" <?php if($output['class_array']['class_type'] == 3){?>selected="selected"<?php }?>>配送</option>
            </select></td>
          <td class="vatop tips"></td>
        </tr>
        <tr>
...
$(document).ready(function(){
	...
	$("#submitBtn").click(function(){
        ...
        /*提交之前 去掉Disbaled 属性  $("select[name=class_type]").attr("disabled",false);  */
