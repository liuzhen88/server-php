    ...
    <table class="tb-type1 noborder search">
      <tbody>
        <tr>
          ...
          <td colspan="3"><input type="text" id="stime" name="stime" class="txt date" value="<?php echo $_GET['stime'];?>">
            <label>~</label>
        ...
        <tr>
         ...
         <th>角色</th>
          <td>
            <select id="role_search" name="role_search">
              <option value="">全部</option>
              <option value="0" <?php if($_GET['role_search'] == '0' ) { ?>selected="selected"<?php } ?>>商家</option>
              <option value="1" <?php if($_GET['role_search'] == '1' ) { ?>selected="selected"<?php } ?>>代理商</option>
            </select>
          </td>
          ...
          <td>
            <select id="paystate_search" name="paystate_search">
              ...
              <option value="0" <?php if($_GET['paystate_search'] == '0' ) { ?>selected="selected"<?php } ?>>未处理</option>
              <option value="2" <?php if($_GET['paystate_search'] == '2' ) { ?>selected="selected"<?php } ?>>已提交</option>
              <option value="3" <?php if($_GET['paystate_search'] == '3' ) { ?>selected="selected"<?php } ?>>提现失败</option>
              <option value="1" <?php if($_GET['paystate_search'] == '1' ) { ?>selected="selected"<?php } ?>>提现成功</option>
            </select>
  ...
  <table class="table tb-type2 nobdb">
    <thead>
      <tr class="thead">
        ...
        <?php if($_GET['role_search'] == '0' ) { ?><th class="align-center">商户</th><?php } ?>
        <th><?php echo $lang['admin_predeposit_membername'];?></th>
    ...
    <tbody>
      ...
      <tr class="hover">
        ...
        <?php if($_GET['role_search'] == '0' ) { ?>  <td class="align-center"><?php echo $v['store_name'];?></td><?php } ?>
        ...
        <td class="align-center"><?php echo $v['available_predeposit'];?></td>
        <td class="align-center"><?php echo str_replace(array('0','2','3','1'), array('未支付','已提交','提现异常','已支付'), $v['pdc_payment_state']); ?></td>
        <td class="w90 align-center">
         <a href="index.php?act=predeposit&op=pd_cash_view&id=<?php echo $v['pdc_id']; ?>" class="edit"><?php echo $lang['nc_view']; ?></a>
          </td>
      </tr>
      <?php } ?>
      <?php }else { ?>
    ...
    <tfoot>
      <tr style="text-align: center;width: 100%"> <td colspan="16" style="text-align: center;width: 100%;height:20px;padding-top: 30px;font-size:16px;"> <div> 提现总和：<?php echo $output['total'];?></div></td></tr>
      <tr>
