...
<div class="page">
  ...
  <form id="goods_class_form" enctype="multipart/form-data" method="post">
    ...
    <table class="table tb-type2">
      ...
      <!-- 商品类别 -->
        <tr>
          <td colspan="2" class="required"><label class="validation">商品类别:</label></td>
        </tr>

        <tr class="noborder">
          <td class="vatop rowform">
          <select name="class_type" id="class_type" size="1" <?php if(isset($output['fag']))  echo "disabled"; ?>   >
            <option value="0" <?php if($output['fag']==0) echo "selected"; ?>>商城</option>
            <option value="1" <?php if($output['fag']==1) echo "selected"; ?>>本土</option>
            <option value="3" <?php if($output['fag']==3) echo "selected"; ?>>配送</option>
          </select>
            </td>
      ...
      <!-- 上级分类 -->
        ...
        <tr class="noborder">
          <td class="vatop rowform">
            ...
            <select name="gc_parent_id" id="gc_parent_id" <?php if(!empty($output['gc_class_name'])) echo "disabled"; ?> >
              <option value="0"><?php echo $lang['nc_please_choose'];?>...</option>
              <?php if(!empty($output['gc_class_name'])){ ?>

                    <option tid="<?php echo $output['class_type']; ?>" selected value="<?php echo $output['tag'];?>"><?php echo $output['gc_class_name'];?></option>
              
              <?php }elseif(!empty($output['parent_list']) && is_array($output['parent_list'])){ ?>
              <?php foreach($output['parent_list'] as $k => $v){ ?>
              <option tid="<?php echo $v['class_type']; ?>" <?php if($output['gc_parent_id'] == $v['gc_id']){ ?>selected='selected'<?php } ?> value="<?php echo $v['gc_id'];?>"><?php echo $v['gc_name'];?></option>
              <?php } ?>
              <?php } ?>
            </select>
        ...
        <tr class="noborder">
          <td class="vatop rowform">
              <input type="text" value="" name="gc_name" id="gc_name" maxlength="20" class="txt">
          </td>
          <td class="vatop tips"></td>
        </tr>
        ...
        <tr class="noborder">
          <td class="vatop rowform"><input id="commis_rate" class="w60" type="text" value="" name="commis_rate">
            %</td>
          <td class="vatop tips">必须为0-100的数</td>
        </tr>
      <tr>
          <td colspan="2" class="required"><label class="validation">平台抽佣:</label></td>
      </tr>
      <tr class="noborder">
          <td class="vatop rowform"><input id="commis_rate" class="w60" type="text" value="" name="distribution_rate">
              %</td>
          <td class="vatop tips">必须为0-100的数</td>
      </tr>
                            ...
                            <div>
                                <dl style="margin:10px 0;">
                                    <dd style="display:inline-block; margin-right:10px;">
                                        <input type="radio" name="gc_type" value="default" checked="checked" />
                                        <span>商品分类</span>
                                    </dd>
                                </dl>
                            </div>
...
$(function(){
	...
	$("#submitBtn").click(function(){
     ...
     /**/
     $("select[name=gc_parent_id]").attr("disabled",false);
     $("select[name=class_type]").attr("disabled",false); 
        ...
        rules : {
            gc_name : {
                ...
                data:{
                    ...
                    gc_class_type: function(){
                       return $('#class_type').find('option:selected').val();
                    },
