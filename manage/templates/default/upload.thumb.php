  ...
  <form method="post" enctype="multipart/form-data" onsubmit="MySubmit();return false;" name="form1">
    ...
    <table class="table tb-type2">
      <tbody>
        ...
        <tr>
          <td colspan="2" class="required"><label for="default_league_store_avatar">跑腿邦店铺头像:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform"><span class="type-file-show"><img class="show_image" src="<?php echo ADMIN_TEMPLATES_URL;?>/images/preview.png">
            <div class="type-file-preview"><img src="<?php echo UPLOAD_SITE_URL.'/'.(ATTACH_COMMON.'/'.$output['list_setting']['default_league_store_avatar']);?>"></div>
            </span><span class="type-file-box">
            <input class="type-file-file" id="default_league_store_avatar" name="default_league_store_avatar" type="file" size="30" hidefocus="true" nc_type="change_default_league_store_avatar" title="<?php echo $lang['default_league_store_avatar'];?>">
            </span></td>
          <td class="vatop tips">100px * 100px</td>
        </tr>
      </tbody>
...
// 模拟默认用户图片上传input type='file'样式
  ...
  // 模拟默认用户图片上传input type='file'样式
  var textButton="<input type='text' name='textfield' id='textfield5' class='type-file-text' /><input type='button' name='button' id='button5' value='' class='type-file-button' />"
  $(textButton).insertBefore("#default_league_store_avatar");
  $("#default_league_store_avatar").change(function(){
    $("#textfield5").val($("#default_league_store_avatar").val());
  });
// 上传图片类型
