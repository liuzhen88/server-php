...
if(!in_array($output['item_info']['item_type'],array('hot'))){
    $show_img_flag = true;
}

?>
<div id="dialog_item_edit_image" style="display:none;">
    <?php if($show_img_flag):?>
  ...
  <form id="form_image" action="">
    ...
    <h4 class="dialog-handle-title">标题：</h4>
      <span>
      <input id="dialog_item_image_title" type="text" class="txt w200 marginright marginbot vatop">
      </span>
        <?php if($show_img_flag):?>
    <h4 class="dialog-handle-title">描述：</h4>
      <span>
      <input id="dialog_item_image_describe" type="text" class="txt w200 marginright marginbot vatop">
      </span>
      ...
      <div>
        ...
        <option value="">-请选择-</option>
          ...
          <option value="good_class">商品分类</option>
        </select>
...
<script id="item_image_template" type="text/html">
    <div nctype="item_image" class="item">
        ...
        <input nctype="image_describe" name="item_data[item][<%=image_name%>][describe]" type="hidden" value="<%=image_describe%>">
        <a nctype="btn_del_item_image" href="javascript:;">删除</a>
    </div>
</script>

<script id="item_text_template" type="text/html">
    <div nctype="item_image" class="item">
        热点:<input nctype="image_type" name="item_data[item][<%=image_title%>][type]" type="hidden" value="<%=image_type%>">
        <input nctype="image_data" name="item_data[item][<%=image_title%>][data]" type="hidden" value="<%=image_data%>">
        <input nctype="image_title" name="item_data[item][<%=image_title%>][title]" type="text" value="<%=image_title%>">
        <a nctype="btn_del_item_image" href="javascript:;">删除</a>
    </div>
</script>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var $current_image_describe = null;
        var old_image = '';
        ...
        var item_type = "<?php echo $output['item_info']['item_type'];?>";
        ...
        $('[nctype="btn_edit_item_image"]').on('click', function() {
            ...
            $current_image_describe = $item_image.find('[nctype="image_describe"]');
            ...
            $('#dialog_item_image_describe').val($current_image_describe.val());
        ...
        var item_type_hot_save = function(){
            var item = {},$message='';
            ...
            if($.trim(item.image_title) ==''){
                $message = '标题不能为空';
            }else if($.trim(item.image_type) ==''){
                $message = '请选择操作类型';

            }else if($.trim(item.image_data) ==''){
                $message = '操作不能为空';
            }
            if($message!=''){
                showError($message);
            ...
            $current_content.append(template.render('item_text_template', item));
        ...
        function edit_item_image_save() {
            ...
            $current_image_describe.val($('#dialog_item_image_describe').val());
        }
        ...
        function add_item_image_save() {
            ...
            item.image_data = $('#dialog_item_image_data').val();
            item.image_title = $('#dialog_item_image_title').val();
            item.image_describe = $('#dialog_item_image_describe').val();
