...
class goods_class_stapleModel extends Model {
	...
	public function autoIncrementStaple1($param, $member_id) {
        $where = array(
            'gc_id1_1' => intval($param['gc_id1_1']),
            'gc_id1_2' => intval($param['gc_id1_2']),
            'gc_id1_3' => intval($param['gc_id1_3']),
            'member_id' => $member_id
        );
        $staple_info = $this->getStapleInfo($where);
        if (empty($staple_info)) {
            $insert = array(
                    'staple_name1' => $param['gc_tag_name1'],
                    'gc_id1_1' => intval($param['gc_id1_1']),
                    'gc_id1_2' => intval($param['gc_id1_2']),
                    'gc_id1_3' => intval($param['gc_id1_3']),
                    'type_id' => $param['type_id'],
