...
class goods_classModel extends Model
{
     ...
     * 缓存数据
     */
    protected $cachedData_0;    //线上分类缓存
    protected $cachedData_1;    //本土分类缓存
    protected $cachedData_3;    //加盟商品分类缓存
    protected $cachedData_all;    //全部分类缓存

    /**
     * 缓存数据 原H('goods_class')形式
     */
    protected $gcForCacheModel_0;
    protected $gcForCacheModel_1;
    protected $gcForCacheModel_3;
    ...
    /**
     * 获取缓存数据
     * @param $class_type int 区分本土分类和店铺分类
     * $class_type 0 :线上；1:本土； all: 全部
     * @return array
     * array(
     *   'data' => array(
     *     // Id => 记录
     ...
     * )
     */
    //本土很电商的分类不能共用一个缓存，...@李熙宇
    public function getCache($class_type=0) {
        $cache_name='cachedData_'.$class_type;
        if ($this->$cache_name) {
            return $this->$cache_name;
        }
        $data = rkcache('gc_class_'.$class_type);
        if (!$data) {
            $data = array();
            $condition=($class_type==='all')?'1=1':array('class_type'=>$class_type);
            ...
            wkcache('gc_class_'.$class_type, $data);
        }
        return $this->$cache_name = $data;
    }
    ...
    public function dropCache() {
        $this->cachedData_0 = null;
        $this->cachedData_1 = null;
        $this->cachedData_3 = null;
        $this->cachedData_all = null;
        $this->gcForCacheModel_0 = null;
        $this->gcForCacheModel_1 = null;
        $this->gcForCacheModel_3 = null;

        dkcache('gc_class_0');
        dkcache('gc_class_1');
        dkcache('gc_class_3');
        dkcache('gc_class_all');
     ...
     * @return array   返回二位数组
     */
    public function getGoodsClassList($condition, $field = '*',$limit=false) {
        $result = $this->table('goods_class')->field($field)->where($condition)->order('gc_parent_id asc,gc_sort asc,gc_id asc')->limit($limit)->select();
        return $result;
    }
     ...
     * @return array   返回数组
     */
    public function getGoodsClassName($condition, $field = '*') {
        $result = $this->table('goods_class')->field($field)->where($condition)->find();
        return $result;
    }
    /**
     * 从缓存获取全部分类
     */
    public function getGoodsClassListAll($class_type=0) {
        $data = $this->getCache($class_type);
     ...
     */
    public function getGoodsClassIndexedListAll($class_type=0) {
        $data = $this->getCache($class_type);
     ...
     */
    public function getGoodsClassListByIds($ids, $class_type=0) {
        $data = $this->getCache($class_type);
        $ret = array();
     ...
     */
    public function getGoodsClassListByParentId($pid,$class_type='all') {
        $data = $this->getCache($class_type);
        $ret = array();
     ...
     * @param int $id 分类id
     */
    public function getGoodsClassInfoById($id, $class_type=0) {
        $data = $this->getCache($class_type);
        return $data['data'][$id];
    }
     ...
     */
    public function getGoodsClassForCacheModel($class_type=0) {
        $cache_model='gcForCacheModel_'.$class_type;
        if ($this->$cache_model)
        ...
        if ($update_all == 1 || !($gc_list = rkcache('all_categories'))) {
            ...
            if (is_array($class_list) && !empty($class_list)) {
                ...
                foreach ($gc_list as $key => $value) {
                    $gc_id = $value['gc_id'];
                    $pic_name = UPLOAD_SITE_URL.'/'.ATTACH_COMMON.'/category-pic-'.$gc_id.'.jpg';
    ...
    public function getChildClass($parent_id){
        ...
        $all_class = $this->getGoodsClassListAll('all');
    ...
    public function getGoodsClassLineForTag($id = 0) {
        if (intval($id)> 0) {
             ...
             * 取当前类别信息
             */
            $class = $this->getGoodsClassInfoById(intval($id),'all');
            ...
            if ($class['gc_parent_id'] != 0) {
                $parent_1 = $this->getGoodsClassInfoById($class['gc_parent_id'],'all');
                if ($parent_1['gc_parent_id'] != 0) {
                    $parent_2 = $this->getGoodsClassInfoById($parent_1['gc_parent_id'],'all');
    ...
    public function adtGetClassInHome(){
        $condition['class_type']=3;
        $condition['gc_id']=array('in','(select gc_id from agg_class_in_home_league)','exp');

        $field[]='gc_id';
        $field[]='gc_name';
        $class=$this->getGoodsClassList($condition,$field,5);
        if(!empty($class)) {
            $gc_id = agg_array_column($class, 'gc_id');
            $image=Model()->table('mb_category')->where(array('gc_id'=>array('in',$gc_id)))->select();
            $image_key_gc_id=array_under_reset($image,'gc_id');
            foreach($class as &$value){
                $value['image']=isset($image_key_gc_id[$value['gc_id']])?$image_key_gc_id[$value['gc_id']]:'';
            }
        }
        return $class;
    }
