    ...
    public function delGroupbuy($condition){
        ...
        if(!empty($groupbuy_list) && $result) {
            foreach ($groupbuy_list as $value) {
                ...
                $path = ATTACH_GROUPBUY.DS.$store_id.DS;
                $QNupload->delFileQny($path.$base_name.'.'.$ext);
                $QNupload->delFileQny($path.$base_name.'_small.'.$ext);
                $QNupload->delFileQny($path.$base_name.'_mid.'.$ext);
                $QNupload->delFileQny($path.$base_name.'_max.'.$ext);
