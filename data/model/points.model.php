...
class pointsModel{
	 ...
	 * 取积分列表总数
	 */
	public function getPointsTotalNum() {
		$list = Db::getCount('points_log');
		return $list;
	}
 ...
 */
	...
	public function getRebateLogList($condition,$page='',$field='*'){
		$condition_str	= $this->getRebateCondition($condition);
		$param	= array();
		$param['table']	= 'rebate_records';
		$param['where']	= $condition_str;
		$param['field'] = $field;
		$param['order'] = $condition['order'] ? $condition['order'] : 'rebate_records.id desc';
		$param['limit'] = $condition['limit'];
		$param['group'] = $condition['group'];
		return Db::select($param,$page);
	}
	...
	public function getRebateSum($condition){
		$condition_str	= $this->getRebateCondition($condition);
		$param	= array();
		$param['table']	= 'rebate_records';
		$param['where']	= $condition_str;
		$param['field'] = 'sum(rebate) as sum';
		$param['order'] = $condition['order'] ? $condition['order'] : 'rebate_records.id desc';
		$param['group'] = $condition['group'];
	 ...
	 * 取返利列表总数
	 */
	public function getRebateLogTotalNum() {
		$list = Db::getCount('rebate_records');
		return $list;
	}
	/**
	 * 积分日志详细信息
	...
	private function getRebateCondition($condition_array)
	{
	    $condition_sql = '';
	    //添加时间 lifengli 20150723 add
	    if ($condition_array['rebate_saddtime']){
	        $condition_sql	.= " and `rebate_records`.add_time >= '{$condition_array['rebate_saddtime']}'";
	    }
	    if ($condition_array['rebate_eaddtime']){
	        $condition_sql	.= " and `rebate_records`.add_time <= '{$condition_array['rebate_eaddtime']}'";
	    }
	    if ($condition_array['rebate_type']){
	        $condition_sql	.= " and `rebate_records`.rebate_type = '{$condition_array['rebate_type']}'";
	    }
	    if ($condition_array['user_type']){
	        $condition_sql	.= " and `rebate_records`.user_type = '{$condition_array['user_type']}'";
	    }
	    if ($condition_array['member_id']){
	        $condition_sql	.= " and `rebate_records`.member_id = '{$condition_array['member_id']}'";
	    }
	    if ($condition_array['store_id']){
	        $condition_sql	.= " and `rebate_records`.store_id = '{$condition_array['store_id']}'";
	    }
	    if ($condition_array['agent_id']){
	        $condition_sql	.= " and `rebate_records`.agent_id = '{$condition_array['agent_id']}'";
	    }
	    if ($condition_array['member_name']){
	        $condition_sql	.= " and `rebate_records`.member_name  like '%{$condition_array['member_name']}%'";
	    }
	    if ($condition_array['store_name']){
	        $condition_sql	.= " and `rebate_records`.store_name  like '%{$condition_array['store_name']}%'";
	    }
	    if ($condition_array['order_sn']){
	        $condition_sql	.= " and `rebate_records`.order_sn = '{$condition_array['order_sn']}'";
	    }
	    if ($condition_array['goods_name']){
	        $condition_sql	.= " and `rebate_records`.goods_name  like '%{$condition_array['goods_name']}%'";
	    }
	    //添加时间 lifengli 20150723 end
	    return $condition_sql;
	}
}
