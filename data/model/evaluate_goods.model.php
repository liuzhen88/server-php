...
class evaluate_goodsModel extends Model {
     ...
     */
    public function getStoreCredit($condition){
        $c = $this->where($condition)->count();
        return $c;   
    }
     ...
     * 获取商品评分信息
     */
    public function getEvaluateGoodsInfo($condition, $field='*') {
    ...
    public function getEvaluateGoodsInfoByGoodsID($goods_id) {
        ...
        if(empty($info)) {
            ...
            if(intval($info['all']) > 0) {
                ...
                $info['star_average'] = round(($star1 + $star2 * 2 + $star3 * 3 + $star4 * 4 + $star5 * 5) / $info['all'],1);
            } else {
     ...
     * 根据商品编号查询商品评价信息
     */
    public function getEvaluateGoodsInfoByStoresCommonID($store_id) {
        $prefix = 'goods_evaluation';
        $info = rcache($store_id, $prefix);
        if(empty($info)) {
            $info = array();
            $count_array = $this->field('count(*) as count,geval_scores')->where(array('geval_storeid' => $store_id))->group('geval_scores')->key(geval_scores)->select();
            $star1 = intval($count_array['1']['count']);
            $star2 = intval($count_array['2']['count']);
            $star3 = intval($count_array['3']['count']);
            $star4 = intval($count_array['4']['count']);
    ...
    /**
     * 根据商品common_id查询商品评价信息
     */
    public function getEvaluateGoodsInfoByGoodsCommonID($goods_commonid) {
        $prefix = 'goods_evaluation_commonid';
        $info = rcache($goods_commonid, $prefix);
        if(empty($info)) {
            $goods_list=Model('goods')->getGoodsList(array('goods_commonid'=>$goods_commonid),'goods_id');
            $goods_ids=array();
            foreach($goods_list as $value){
                $goods_ids[]=$value['goods_id'];
            }
            $info = array();
            $count_array = $this->field('count(*) as count,geval_scores')->where(array('geval_goodsid' => array('in',implode(',',$goods_ids))))->group('geval_scores')->key('geval_scores')->select();
            $star1 = intval($count_array['1']['count']);
            $star2 = intval($count_array['2']['count']);
            $star3 = intval($count_array['3']['count']);
            $star4 = intval($count_array['4']['count']);
            ...
            $info['bad'] = $star1;
            $info['all'] = $star1 + $star2 + $star3 + $star4 + $star5;
            $info['image'] = $this->field('count(*) as count')->where(array(
                'geval_goodsid' => array('in',implode(',',$goods_ids))
                ,'geval_image'  => array('NEQ','')
            ))->count();
            if(intval($info['all']) > 0) {
                $info['good_percent'] = intval($info['good'] / $info['all'] * 100);
                $info['normal_percent'] = intval($info['normal'] / $info['all'] * 100);
