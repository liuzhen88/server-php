...
class addressModel extends Model {
     ...
     * 爱大腿，获取一条地址
     */
    public function adt_getDefaultAddressInfo($condition = array(), $order = 'is_default desc,address_id desc'){
        return Model('address_league')->where($condition)->order($order)->find();
    }

    /**
     * 爱大腿，获取附近的地址
     */
    public function adt_getAddressNearby($member_id,$lat,$lng){
        ...
        $address=Model('address_league')->field('*'.$distance_sql)->where($condition)->order('distance asc')->find();
        if(!empty($address) && $address['distance']>ADT_NEARE_ADDRESS_DISTANCE){
            $address=array();
        }
        return $address;
    }
	 ...
	 * @return bool 布尔类型的返回结果
	 */
	public function adt_editAddress($update, $condition){
        return Model('address_league')->where($condition)->update($update);
	}
	/**
	 * 验证地址是否属于当前用户
	 *
