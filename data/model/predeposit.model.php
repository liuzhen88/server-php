...
class predepositModel extends Model {
    ...
    public function makeSnBymember($member_id) {
        return mt_rand(10,99)
        . sprintf('%010d',time() - 946656000)
        . sprintf('%03d', (float) microtime() * 1000)
     ...
     * @param unknown $condition
     */
    public function getPdCashCount($condition = array(),$role_search = '') {
        if ($role_search == ''){
            return $this->table('pd_cash,member')->join('inner')->on('pd_cash.pdc_member_id=member.member_id')->where($condition)->count();
        }
        elseif ($role_search == '0'){
            $condition['seller.is_admin'] = 1;
            return $this->table('pd_cash,seller,member,store')->join('inner,inner,inner')->on('pd_cash.pdc_member_id=seller.member_id,seller.member_id=member.member_id,store.store_id=seller.store_id')->where($condition)->count();
        }
        elseif ($role_search == '1'){
            return $this->table('pd_cash,agent,member')->join('inner,inner')->on('pd_cash.pdc_member_id=agent.member_id,member.member_id=agent.member_id')->where($condition)->count();
        }
}
    /**
     * 提现总额
     * @param unknown $condition
     */
    public function getPdCashSum($condition = array(),$role_search = '') {
        if ($role_search == ''){
            return $this->table('pd_cash,member')->join('inner')->on('pd_cash.pdc_member_id=member.member_id')->field('sum(pdc_amount) as sum')->where($condition)->find();
        }
        elseif ($role_search == '0'){
            $condition['seller.is_admin'] = 1;
            return $this->table('pd_cash,seller,member,store')->join('inner,inner,inner')->on('pd_cash.pdc_member_id=seller.member_id,seller.member_id=member.member_id,store.store_id=seller.store_id')->field('sum(pdc_amount) as sum')->where($condition)->find();
        }
        elseif ($role_search == '1'){
            return $this->table('pd_cash,agent,member')->join('inner,inner')->on('pd_cash.pdc_member_id=agent.member_id,member.member_id=agent.member_id')->field('sum(pdc_amount) as sum')->where($condition)->find();
        }
    }
    ...
    public function changePd($change_type,$data = array()) {
        ...
        switch ($change_type){
            case 'order_pay':
                ...
                $data_log['lg_mark'] = json_encode($lg_mark);
            ...
            case 'order_delete':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_freeze_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '删除订单，解冻预存款，订单号: '.$data['order_sn'];
                $lg_mark = array(
                    'order_sn'=>$data['order_sn'],
                );
                $data_log['lg_mark'] = json_encode($lg_mark);
            ...
            case 'cash_del':
                ...
                $data_msg['desc'] = $data_log['lg_desc'];
                break;
            /**  add by chenyifei 2015-07-24 */ 
              
            case 'rebate_pay':
            case 'settle_account':
            case 'rebate_get':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = $data['order_sn'];
                $data_log['lg_mark'] = isset($data['lg_mark']) ? json_encode($data['lg_mark']) : '';
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
            ...
            case 'distribution_get':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = $data['order_sn'];
                $data_log['lg_mark'] = isset($data['lg_mark']) ? json_encode($data['lg_mark']) : '';
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
                if ($data['is_store_distribution'] == 1)
                {
                    //本土商户分销总额更新
                    Model('store')->editStore(array('distribution_commission'=>array('exp','distribution_commission+'.$data['amount'])), array('store_id'=>$data['store_id']));
                }
                else 
                {
                    $data_pd['distribution_points'] = array('exp','distribution_points+'.$data['distribution_goods_money']);  //个人分销总额更新
                    $data_pd['distribution_commission'] = array('exp','distribution_commission+'.$data['amount']);  //个人分销总额更新
                }
                if (isset($data['upgrade_first_distribution']) && $data['upgrade_first_distribution'] == 1)
                {
                    $data_pd['is_distribution'] = 1;
                }
                $data_msg['av_amount'] = $data['amount'];
                $data_msg['freeze_amount'] = 0;
                $data_msg['desc'] = $data_log['lg_desc'];
     ...
     * @param string $order
     */
    public function getPdCashList($condition = array(), $pagesize = '', $fields = '*', $order = '', $limit = '',$role_search = '') {
        if ($role_search == ''){
            return $this->table('pd_cash,member')->join('inner')->on('pd_cash.pdc_member_id=member.member_id')->where($condition)->field('pd_cash.*,member.available_predeposit')->order($order)->limit($limit)->page($pagesize)->select();
        }
        elseif ($role_search == '0'){
            $condition['seller.is_admin'] = 1;
            return $this->table('pd_cash,seller,member,store')->join('inner,inner,inner')->on('pd_cash.pdc_member_id=seller.member_id,seller.member_id=member.member_id,store.store_id=seller.store_id')->where($condition)->field('pd_cash.*,member.available_predeposit,store.store_name')->order($order)->limit($limit)->page($pagesize)->select();
        }
        elseif ($role_search == '1'){
            return $this->table('pd_cash,agent,member')->join('inner,inner')->on('pd_cash.pdc_member_id=agent.member_id,member.member_id=agent.member_id')->where($condition)->field('pd_cash.*,member.available_predeposit')->order($order)->limit($limit)->page($pagesize)->select();
        }
    }
