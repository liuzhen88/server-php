...
class refund_returnModel extends Model {
     ...
     * @since  2015-10-29
     */
    public function editRefundReturn2($refund_id, $refund_array, $order = array(), $goods = array()) {
        if (!empty($order) && is_array($order)) {
            $refund_array['order_id'] = $order['order_id'];
            $refund_array['order_sn'] = $order['order_sn'];
            $refund_array['store_id'] = $order['store_id'];
        ...
        //echo $this->table('refund_return')->_sql();
        // 发送商家提醒
        $param = array();
        if (intval($refund_array['refund_type']) == 1) {    // 退款
            $param['code'] = 'refund';
        } else {    // 退货
            $refund_array['refund_state'] = 1;//非第一次申请设置为处理中订单2015-11-09
            $param['code'] = 'return';
        }
        $refund_id = $this->table('refund_return')->where($where)->update($refund_array);
        $param['store_id'] = $order['store_id'];
        $type = $refund_array['order_lock'] == 2 ? '售前' : '售后';
        $param['param'] = array(
            'type' => $type,
     ...
     * @return array
     */
    public function getRefundList($condition = array(), $page = '',$field = '*') {
        $condition['refund_type'] = '1'; //类型:1为退款,2为退货
        $result = $this->getRefundReturnList($condition, $page,$field);
        return $result;
    }
    ...
    public function getGoodsRefundList($order_list = array(), $order_refund = 0) {
        ...
        if (!empty($refund_list) && is_array($refund_list)) {
            ...
            foreach ($refund_list as $key => $value) {
                ...
                if (empty($refund_goods[$order_id][$goods_id])) {
                    ...
                    //二次处理退货订单
                    $value = $this->_set_refund_detail($value); 
                    $refund_goods[$order_id][$goods_id] = $value;
                    
                    if ($order_refund > 0) {//订单下的退款退货所有记录
                        $order_list[$order_id]['refund_list'] = $refund_goods[$order_id];
                    }
                    if(isset($order_list[$order_id]['extend_order_goods'])){
                        if($refund_list[0]['seller_state']==2){
                            $status = 1;
                        }else{
                            $order_list[$order_id]['extend_order_goods'][0]['refund_state']=$refund_list[0]['refund_state'];
                            $order_list[$order_id]['extend_order_goods'][0]['refund_type']=$refund_list[0]['refund_type'];
                            $order_list[$order_id]['extend_order_goods'][0]['seller_state']=$refund_list[0]['seller_state'];

                            $order_list[$order_id]['extend_order_goods'][0]['refund_timce']=count($refund_list);
                            $order_list[$order_id]['extend_order_goods'][0]['isrefund']=1;
                            $order_list[$order_id]['extend_order_goods'][0]['refund_id']=$value['refund_id'];
                        }
        ...
        if (!empty($order_list) && is_array($order_list)) {
            foreach ($order_list as $key => $value) {
                ...
                $order_paid = $model_trade->getOrderState('order_paid'); //订单状态20:已付款 order_paid
                $payment_code = $value['payment_code']; //支付方式
                
                if ($order_state == $order_paid && $payment_code != 'offline') {//已付款未发货的非货到付款订单可以申请取消
                    if($status==1  || (isset($_REQUEST['client_type']) && strtolower($_REQUEST['client_type']) != 'wap')){
//                            unset($order_list[$order_id]['refund']) ;
                        if(!empty($goods_list) && is_array($goods_list)){
                            ...
                            foreach ($goods_list as $k => $v) {
                               ...
                               if (!empty($refund_goods[$order_id][$goods_id])) {
                                   ...
                                   $reason_id = $refund_goods[$order_id][$goods_id]['reason_id'];
                                   $reason_detail = Model()->table('refund_reason')->where(array('reason_id' =>$reason_id ))->find();
                                   if (!empty($reason_detail)) {
                                          $v['extend_refund']['reason_info'] = $reason_detail['reason_info'];
                                   }
                               }
                               else {
                                    $seller_state = $refund_goods[$order_id][0]['seller_state']; //卖家处理状态:1为待审核,2为同意,3为不同意
                                   if ($seller_state == 3) {
                                       $order_list[$order_id]['extend_complain'][0] = '1'; //不同意可以发起退款投诉
                                   } else {
                                       $v['refund'] = '0'; //已经存在处理中或同意的商品不能再操作
                                   }
                                   $v['extend_refund'] = $refund_goods[$order_id][0];
                                   $reason_id = $refund_goods[$order_id][0]['reason_id'];
                                   $reason_detail = Model()->table('refund_reason')->where(array('reason_id' =>$reason_id ))->find();
                                   if (!empty($reason_detail)) {
                                          $v['extend_refund']['reason_info'] = $reason_detail['reason_info'];
                                   }
    ...
    public function getRefundGoodsId($condition, $master=FALSE) {
        $data = $this->table('refund_return')->where($condition)->master($master)->select();
        $result = array();
        if (!empty($data)) {
    ...
    public function getRefundGoods($condition, $master=FALSE) {
        $data = $this->table('refund_return')->where($condition)->master($master)->select();
     ...
     * 二次设置退货信息
     * 
     * @param array $refund_info 退货信息
     * @return array
     */
    private function _set_refund_detail($refund_info = array()) {
        
        // 图片凭证
        if ($refund_info['pic_info']) {
            $refund_info['pic_info'] = unserialize($refund_info['pic_info']);
        }
        
        $refund_info['refund_time'] = 0;
        // 无退货
        if (empty($refund_info)) {
            $refund_info['refund_state_tip']  = '无退款';
            $refund_info['seller_state_tip']  = '';
            $refund_info['refund_code']       = '0'; 
        }
        if($refund_info['refund_type'] == 1 || (isset($_REQUEST['client_type']) && strtolower($_REQUEST['client_type']) == 'wap'))
        {
            // 商家审核
            if ($refund_info['refund_state'] == 1 && $refund_info['seller_state'] == 1) {
                $refund_info['refund_state_tip']  = '退款中';
                $refund_info['seller_state_tip']  = '待商家审核';
                $refund_info['refund_code']       = '1';
            }
            
            // 商家同意
            elseif ($refund_info['refund_state'] == 2 && $refund_info['seller_state'] == 2) {
                $refund_info['refund_state_tip']  = '退款中';
                $refund_info['seller_state_tip']  = '待平台审核';
                $refund_info['refund_code']       = '1';
            }
            
            // 商家同意且平台通过审核
            elseif ($refund_info['refund_state'] == 3 && $refund_info['seller_state'] == 2) {
               $refund_info['refund_state_tip']   = '退款成功';
               $refund_info['seller_state_tip']   = '商家同意并通过审核'; 
               $refund_info['refund_code']        = '2';
               $refund_info['refund_time']        = $refund_info['admin_time']; //平台处理时间即退款成功时间
            }
            
            // 商家拒绝
            elseif ($refund_info['refund_state'] == 3 && $refund_info['seller_state'] == 3) {
               $refund_info['refund_state_tip']   = '退款失败';
               $refund_info['seller_state_tip']   = '商家拒绝'; 
               $refund_info['refund_code']        = '3';
                $refund_info['refund_time']       = $refund_info['seller_time']; //卖家处理时间即退款失败时间
            }
        }
        else
        {
            ...
            if ($refund_info['seller_state'] == 1) {
                $refund_info['refund_state_tip']  = '退货中';
                $refund_info['seller_state_tip']  = '待商家审核';
                $refund_info['refund_code']       = '11';
            }
            // 商家审核
            /*
`refund_type` '申请类型:1为退款,2为退货,默认为1',
`seller_state` '卖家处理状态:1为待审核,2为同意,3为不同意,默认为1',
`refund_state` '申请状态:1为处理中,2为待管理员处理,3为已完成,默认为1',
`return_type` '退货类型:1为不用退货,2为需要退货,默认为1',
`goods_state` '物流状态:1为待发货,2为待收货,3为未收到,4为已收货,默认为1',
             */
            if ($refund_info['seller_state'] == 2) {
                   //如果是卖家同意，但是平台在处理中
            if($refund_info['refund_state'] == 1 || $refund_info['refund_state']==2){
                  if($refund_info['refund_type'] == 2){
                    
                        //这是退货卖家同意填写退货物流
                        if($refund_info['goods_state']==1){
                            $refund_info['refund_state_tip']  = '退货中';
                            $refund_info['seller_state_tip']  = '等待商家收货';
                            $refund_info['refund_code']       = '11';
                        }
                        if($refund_info['goods_state']==2){
                          //这是商家同意用户退货，用户也填写了申请等待商家收货
                            $refund_info['refund_state_tip']  = '退货中';
                            $refund_info['seller_state_tip']  = '等待商家收货';
                            $refund_info['refund_code']       = '11';
                        }
                        if($refund_info['goods_state']==3){
                          //用户退货了，但是商家没有收到货
                          //后台暂时无此功能  
                        }
                        
                        if($refund_info['goods_state']==4){
                          //商家已经收货,待确认
                ...
                }elseif($refund_info['refund_state']==3){
                  //退款成功
                   $refund_info['refund_state_tip']   = '退货成功';
                   $refund_info['seller_state_tip']   = '商家同意并通过审核'; 
                   $refund_info['refund_code']        = '12';
                   $refund_info['refund_time']        = $refund_info['admin_time']; //平台处理时间即退款成功时间
                }
            ...
            if ($refund_info['seller_state'] == 3) {
                $refund_info['refund_state_tip']  = '退货失败';
                $refund_info['seller_state_tip']  = '商家拒绝';
                $refund_info['refund_code']       = '13';
                $refund_info['refund_time']       = $refund_info['seller_time']; //卖家处理时间即退款失败时间
            }
        }
        // 前端退货时处理，退货倒计时，倒计时需要延迟一天  单位：天
        $refund_info['refund_confirm_day']  = ORDER_REFUND_CONFIRM_DAY + 1;
     ...
     * @return array|bool
     */
    public function getOrderBySN($condition, $field = [])
    {
        $refund_order = $this->getRefundList($condition,'',$field[0]);
        if(empty($refund_order)) return false;
        //过滤以便查询有退款申请的订单,去查询退款金额,退款理由
        $order = array();
        foreach ($refund_order as $index => $item) {
            $order[$item['order_sn']] = $item;
        }

        /** @var orderModel $order_model */
        $order_model = Model('order');
        $condition = array();
        $condition['order_sn'] =  array('in',array_keys($order));
        $order_list = $order_model->getOrderList($condition,'','order_sn,pay_sn');
        $pay_sn = array();
        foreach ($order_list as $index => $item) {
            $pay_sn [$item['pay_sn']] = $item['order_sn'];
            $order[$item['order_sn']]['pay_sn'] = $item['pay_sn'];
        }

        $condition = array();
        $condition['pay_sn'] =  array('in',array_keys($pay_sn));
        $order_pay_list = $order_model->getOrderPayList($condition);

        foreach ($order_pay_list as $index => $item) {
            $temp_pay_sn = $item['pay_sn'];
            $order[$pay_sn[$temp_pay_sn]]['api_trade_no'] = $item['api_trade_no'];
        }
        return $order;
    ...
    /**
     * 根据支付单号获取订单数据
     * @param string $api_trade_no 支付单号
     * @return mixed
     */
    public function getOrderByTrade($api_trade_no)
    {
        $field = 'order.pay_sn,order.order_id,order.order_sn,order.order_amount,order_pay.api_trade_no';
        $condition['order_pay.api_trade_no'] = $api_trade_no;
        $order = $this->table('order','order_pay')->join('inner')->on('order_pay.pay_sn=order.pay_sn')->field($field)->where($condition)->find();
        return $order;
    }
