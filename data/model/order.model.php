...
class orderModel extends Model {
     ...
     * @return unknown
     */
    public function getOrderInfo($condition = array(), $extend = array(), $fields = '*', $order = '',$group = '', $master = false) {
        $order_info = $this->table('order')->field($fields)->where($condition)->group($group)->order($order)->master($master)->find();
        if (empty($order_info)) {
            return array();
        }
        ...
        if (in_array('store',$extend)) {
            if(isset($order_info['order_type']) && $order_info['order_type'] ==3){
                $order_info['extend_store'] = Model('store')->getStoreInfo(array('store_id' => $order_info['league_store_id']));
            }else {
     ...
     * @param unknown $condition
     */
    public function getOrderStateSendingCount($condition = array()) {
        $condition['order_state'] = ORDER_STATE_SENDING;
        return $this->getOrderCount($condition);
    }
     ...
     * @param string $order
     */
    public function getOrderGoodsCount($condition = array()) {
        return $this->table('order_goods')->where($condition)->count();
    }
     ...
     * 跑腿邦，添加配送评价
     */
    public function addOrderEvaluate($data) {
        return $this->table('evaluate_league')->insert($data);
    }

    /**
     * 跑腿邦，取得配送评价
     */
    public function getOrderEvaluate($condition = array(), $fields = '*', $order = '') {
        return $this->table('evaluate_league')->where($condition)->field($fields)->order($order)->find();
    }
	 ...
	 * @return Ambigous <multitype:, unknown>
	 */
    public function getOrderAssistList($condition) {
        return $this->table('order_league_assist')->where($condition)->select();
    }
    ...
    public function getOrderOperateState($operate,$order_info){
        ...
        switch ($operate) {
	       ...
	       case 'payment':
        	...
        	//放入回收站
        	case 'delete':
        	    $state = in_array($order_info['order_state'], array(ORDER_STATE_CANCEL,ORDER_STATE_NEW,ORDER_STATE_SUCCESS)) && $order_info['delete_state'] == 0;
        	    break;

        	//永久删除、从回收站还原
        	case 'drop':
        	case 'restore':
        	    $state = in_array($order_info['order_state'], array(ORDER_STATE_CANCEL,ORDER_STATE_NEW,ORDER_STATE_SUCCESS)) && $order_info['delete_state'] == 1;
        	    break;
     ...
     * 添加协调订单记录
     */
    public function addAssistOrderLog($data) {
        $data['add_time'] = TIMESTAMP;
        $data['status'] = 0;
        return $this->table('order_league_assist')->insert($data);
    }

    /**
     * 处理协调订单
     */
    public function editAssistOrderLog($data,$condition) {
        $data['audit_time'] = TIMESTAMP;
        $data['status'] = 1;
	    return $this->table('order_league_assist')->where($condition)->update($data);
    }

    /**
     * 获取协调订单记录列表
     */
    public function getAssistOrderLogList($condition,$field = '*',$order = 'id desc',$master=false) {
        return $this->table('order_league_assist')->where($condition)->field($field)->master($master)->group('order_id')->order($order)->limit(1000)->select();
    }

    /**
     * 获取协调订单记录
     */
    public function getAssistOrderLog($condition,$order = 'id desc') {
        return $this->table('order_league_assist')->where($condition)->order($order)->find();
    }

    /**
     * 获取商铺本月送达率
     */
    public function getOrderSendPercent($league_store_id) {
        $query = 'SELECT count(*) FROM agg_order WHERE order_state=40 AND order_type=3 AND league_store_id='.$league_store_id.' AND hope_time > (unix_timestamp(now())-(60*60*24*30))';
        $all = $this->query($query);
        if($all[0]['count(*)']) {
            $send = $this->query($query . ' AND hope_time > finnshed_time');
            return round($send[0]['count(*)'] / $all[0]['count(*)'], 4) * 100;
        }
        return false;
    }
