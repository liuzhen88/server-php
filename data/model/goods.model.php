...
class goodsModel extends Model{
    ...
    public function getGoodsListSort($condition, $field = '*', $group = '',$order = '', $limit = 0, $page = 0, $lock = false, $count = 0) {
        return $this->table('goods')->field($field)->where($condition)->group($group)->order($order)->limit($limit)->page($page, $count)->lock($lock)->select();
    }
    /**
     * 根据条件来查看所有商品
     ...
     */
    public function getGoodsListId($condition, $field = '*', $group = '',$order = '', $limit = 0, $page = 0, $lock = false, $count = 0) {
        return $this->table('goods')->field($field)->where($condition)->group($group)->order($order)->limit($limit)->page($page, $count)->lock($lock)->select();
    }
    ...
    public function getGoodsCommonList($condition, $field = '*', $page = 10, $order = 'goods_commonid desc') {
        $condition = $this->_getRecursiveClass($condition);
        $rs = $this->table('goods_common')->field($field)->where($condition)->order($order)->page($page)->select();
        return $rs;
    }
    ...
    public function getGoodsSellingById($goods_id) {
     ...
     * @return array
     */
    public function getGoodeCommonList($condition, $field = '*') {
        return $this->table('goods_common')->field($field)->where($condition)->select();
    }
    ...
    public function getGoodsCommonCount($condition) {
        $res=$this->table('goods_common')->where($condition)->count();
        $this->table('goods');
        return $res;
    }
...
//                @unlink(BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.$val['store_id'].DS.$val['goods_id'].'.png');
                ...
                $QNupload->delFileQny(ATTACH_STORE.DS.$val['store_id'].DS.$val['goods_id'].'.png');
                // 删除商品缓存
     ...
     * @return boolean
     */
    public function delDisGoodsAll($condition) {
        $goods_list = $this->getGoodsList($condition, 'goods_id,goods_commonid,store_id');
        if (empty($goods_list)) {
            return false;
        }
        ...
        // agg_distribution_goods
        $this->table('distribution_goods')->where(array('goods_commonid' => array('in', $commonid_array)))->delete();

        // 删除分销商品表数据
        $this->table('goods_distribution')->where(array('goods_id' => array('in', $goodsid_array)))->delete();

        //分销商品的删除 删除收藏表
        $this->table('distribution_goods_favorites')->where(array('goods_commonid' => array('in', $commonid_array)))->delete();

        //分销商品的删除  公共表的数据删除
        $this->table('goods_common_distribution')->where(array('goods_commonid' => array('in', $commonid_array)))->delete();
     ...
     * 编辑分销商品 为不分销时
     */
    public  function  delDisgood($commonid_array){

        // agg_distribution_goods
        $this->table('distribution_goods')->where(array('goods_commonid' => array('in', $commonid_array)))->delete();

        // 删除分销商品表数据
        $this->table('goods_distribution')->where(array('goods_commonid' => array('in', $commonid_array)))->delete();
     ...
     * @tool   :phpstom
     */
    public function delDisGoodsNoLock($condition) {
        $condition['goods_lock'] = 0;
        $common_array = $this->getGoodsCommonList($condition, 'goods_commonid', 0);
        $common_array = array_under_reset($common_array, 'goods_commonid');
        $commonid_array = array_keys($common_array);
        return $this->delDisGoodsAll(array('goods_commonid' => array('in', $commonid_array)));
    }
     ...
     * @return array
     */
    public function getGoodsInfoSort($sort, $fields = '*',$cache_time) {
        $goods_info = $this->_rGoodsCache('_tj_'.$sort['city_id'].'_'.$sort['gc_id_1'], $fields);
        if (empty($goods_info)) {
            $goods_info = $this->getGoodsListSort($sort,$fields,'','goods_salenum desc', 4);
            $goods_json = json_encode($goods_info);
            $this->_wGoodsCacheSort('_tj_'.$sort['city_id'].'_'.$sort['gc_id_1'], array('goods_info'=>$goods_json),$cache_time);
            if($fields!='*'){
                $fields=explode(',',$fields);
                foreach($goods_info as $key=> $value){
                    if(!in_array($key,$fields)){
            ...
            $goods_info = json_decode($goods_info['goods_info'],true);
        }
        return $goods_info;
    }
     ...
     * @return boolean
     */
    private function _wGoodsCacheSort($goods_id, $goods_info) {
        return wcache($goods_id, $goods_info, 'goods',1440);
    }
    ...
    public function getGoodsDetail($goods_id) {
        ...
        $goods_info['spec_value'] = mb_unserialize($goods_info['spec_value']);
        $goods_info['spec_name'] = mb_unserialize($goods_info['spec_name']);
        $goods_info['goods_spec'] = mb_unserialize($goods_info['goods_spec']);
        $goods_info['goods_attr'] = mb_unserialize($goods_info['goods_attr']);
        ...
        if ($goods_info['mobile_body'] != '') {
            $mobile_body_array = mb_unserialize($goods_info['mobile_body']);
            $mobile_body = '';
...
//                                $image_url=qnycthumbType($image_url,360,true);
                            ...
                            if('android'==strtolower($_REQUEST['client_type'])||'ios'==strtolower($_REQUEST['client_type'])){
                                ...
                                $mobile_body .= '<img data-echo="' . $image_url . '" class="lazyLoad" src="../images/pre_img.jpg" >';
                            }
                            $val['value']=$image_url;
                    	    break;
                    }
        ...
        $goods_info['mobile_body_pre'] = $mobile_body_pre;
        ...
        if (!empty($image_more)) {
            foreach ($image_more as $val) {
				...
				$goods_image[] = array(cthumb($val['goods_image'], 60, $goods_info['store_id'],true),cthumb($val['goods_image'], 360, $goods_info['store_id'],true),cthumb($val['goods_image'], 1280, $goods_info['store_id'],true));
                $temp_image = cthumb($val['goods_image'], '', $goods_info['store_id']);
//                $temp_image = str_ireplace("?imageView2/1/w/360/h/360", '-w480h480wmfill2', $temp_image);
                $temp_image=$temp_image.'-toppic2';
                $goods_image_mobile[] = $temp_image;
            }
        } else {
            $goods_image[] = "{ title : '', levelA : '".thumb($goods_info, 60,true)."', levelB : '".thumb($goods_info, 360,true)."', levelC : '".thumb($goods_info, 360,true)."', levelD : '".thumb($goods_info, 1280,true)."'}";
            $temp_image=thumb($goods_info, '');
            $goods_image_mobile[] = $temp_image.'-toppic2';
        }
        ...
        if (!empty($goods_info['groupbuy_info'])) {
            ...
            $goods_info['promotion_price'] = del0($goods_info['groupbuy_info']['groupbuy_price']);
        ...
        if (C('cache_open')) {
            $goods_info_cache =  $this->_rGoodsCache($goods_id, 'goods_name');
     ...
     * @return array
     */
    public function getGoodsCommonByDistributionList($condition, $field = '*', $page = 10, $order = 'goods_common.goods_commonid desc') {
        //先把goods_commonid查出来
        $condition = $this->_getRecursiveClass($condition);
        $cond_arr  = array();
        if (!empty($condition)) {
            foreach ((array)$condition as $key => $value) {
                if ($key == 'distribution_recommend') {
                   $cond_arr['goods_common_distribution.is_recommend'] = $value;
                } else {
                    $cond_arr['goods_common.' . $key] = $value;
                }
            }
        }
        
        $rs = $this->table('goods_common,goods_common_distribution')
                   ->field('goods_common.*,goods_common_distribution.is_recommend AS distribution_recommend ')
                   ->join('left join')
                   ->on('goods_common.goods_commonid=goods_common_distribution.goods_commonid')
                   ->where($cond_arr)
                   ->order($order)
                   ->page($page)
                   ->select();
     ...
     *
     * SELECT *
    FROM `agg_goods` AS t1 JOIN (
    SELECT ROUND(RAND() * ((SELECT MAX(goods_id) FROM `agg_goods`)-(SELECT MIN(goods_id) FROM `agg_goods`))+(SELECT MIN(goods_id) FROM `agg_goods`)) AS goods_id
    from `agg_goods` limit 50) AS t2 on t1.goods_id=t2.goods_id  WHERE ( good_type = '1' ) AND ( goods_state = '1' ) AND ( goods_verify = '1' )
    ORDER BY t1.goods_id LIMIT 30;
     */
    public function getGoodsRandom($condition,$field="*",$limit=3){
        $condition['goods_state']=1;
        $condition['goods_verify']=1;
        //商城商品太少，不能按照以下逻辑取随机数
        if(isset($condition['good_type']) && $condition['good_type']==2){
            return $this->table('goods')->where($condition)->limit($limit)->field($field)->order('rand()')->group('goods_commonid')->select();
        }

        //随机取50个id
        $res=$this->table('goods')->where($condition)->field('min(goods_id) min')->find();
        $min=$res['min'];
        $res=$this->table('goods')->where($condition)->field('max(goods_id) max')->find();
        $max=$res['max'];
        $range=array_flip(range($min,$max));
        if(rand(1,10)>5){
            $range=array_reverse($range);
        }
        $rand_goods_id=array_rand($range,$limit*5);
        $condition['goods_id']=array('in',$rand_goods_id);

        $res=$this->table('goods')->where($condition)->limit($limit)->field($field)->select();
        return $res;
    }
     ...
     * 纯粹商品列表数据
     */
    public function getGoodsListSimple($condition,$field="*",$limit=3,$order='goods_id desc'){
        return $this->table('goods')->where($condition)->limit($limit)->field($field)->order($order)->group('goods_commonid')->select();
    }
     ...
     * @return mixed
     */
    public function adtGetGoodsList($condition,$field="*",$order='goods_id desc', $limit = 0){
//        $on='goods.goods_id=goods_league.goods_id';
        $res=$this->table('goods_league')->where($condition)->limit($limit)->field($field)->order($order)->select();
        $this->table('goods');
        return $res;
    }
     ...
     * 跑腿邦，批量查商品详情
     */
    public function adtGetAllGoodsDetail($goods_ids_array,$store_id){
        $goods_league_info_all = Model('goods_league')->where(array('goods_id' => array('in',$goods_ids_array), 'league_store_id' => $store_id))->select();
        if (empty($goods_league_info_all)) {
            return array();
        }
        $goods_commonid_array=agg_array_column($goods_league_info_all,'goods_commonid');
        $goods_common_info_all = Model('goods_common')->field(array('mobile_body','validity_time','brand_name','goods_commonid'))->where(array('goods_commonid' =>array('in',$goods_commonid_array)))->select();
        if (!$goods_common_info_all){
            return array();
        }
        $goods_info_all=array();
        $goods_league_info_all=array_under_reset($goods_league_info_all,'goods_id');
        $goods_common_info_all=array_under_reset($goods_common_info_all,'goods_commonid');
        foreach($goods_ids_array as $value){
            $goods_info=array();
            $goods_image_mobile=array();
            if(isset($goods_league_info_all[$value])){
                $goods_league_info=$goods_league_info_all[$value];
                $goods_info['goods_id'] = $goods_league_info['goods_id'];
                $goods_info['goods_commonid'] = $goods_league_info['goods_commonid'];
                $goods_info['goods_name'] = $goods_league_info['goods_name'];
                $goods_info['goods_image'] = $goods_league_info['goods_image'];
                $goods_info['goods_size'] = $goods_league_info['goods_size'];
                $goods_info['goods_storage'] = $goods_league_info['league_goods_storage'];
                $goods_info['goods_verify'] = $goods_league_info['league_goods_verify'];
                $goods_info['goods_promotion_price'] = del0($goods_league_info['league_goods_promotion_price']);
                $goods_info['goods_promotion_type'] = $goods_league_info['league_goods_promotion_type'];
                $goods_info['goods_price'] = del0($goods_league_info['league_goods_price']);
                // 商品图片
                $goods_image_mobile[] = cthumb($goods_info['goods_image'], '', $goods_league_info['store_id']) . '-ptbw480wm';
            }else{
                $goods_info['goods_id'] = 0;
                $goods_info['goods_commonid'] = 0;
                $goods_info['goods_name'] = '';
                $goods_info['goods_image'] = '';
                $goods_info['goods_size'] = '';
                $goods_info['goods_storage'] = 0;
                $goods_info['goods_verify'] = 0;
                $goods_info['goods_promotion_price'] = 0;
                $goods_info['goods_promotion_type'] = 0;
                $goods_info['goods_price'] = 0;
                $goods_image_mobile[]='';
            }
            if(isset($goods_common_info_all[$goods_info['goods_commonid']])){
                $goods_common_info=$goods_common_info_all[$goods_info['goods_commonid']];
                $goods_info['validity_time']=$goods_common_info['validity_time'];
                $goods_info['brand_name']=$goods_common_info['brand_name'];
                // 解析手机端商品详情信息
                $goods_info['mobile_body'] = $this->mobile_bodySeriToHtml($goods_common_info['mobile_body'], true);
            }else{
                $goods_info['validity_time']='';
                $goods_info['brand_name']='';
                $goods_info['mobile_body']='';
            }
            unset($goods_info['goods_commonid']);
            $out_data[$value]['good_info'] = $goods_info;
            $out_data[$value]['good_image'] = $goods_image_mobile;
        }
        return $out_data;
     ...
     * adt 商品详情
     */
    public function adtGetGoodsDetail($goods_id,$store_id){
//        $out_data = $this->adt_rGoodsCache($goods_id,$store_id);
//        if (empty($out_data)) {
            $goods_league_info = Model('goods_league')->where(array('goods_id' => $goods_id, 'league_store_id' => $store_id))->find();
            if (empty($goods_league_info)) {
                return array();
            }
            $goods_common_info = Model('goods_common')->field(array('mobile_body','validity_time','brand_name'))->where(array('goods_commonid' => $goods_league_info['goods_commonid']))->find();
            ...
            // 商品图片
            $goods_image_mobile[] = cthumb($goods_info['goods_image'], '', $goods_league_info['store_id']) . '-ptbw480wm';
/*            $image_more = $this->getGoodsImageByKey($goods_league_info['goods_commonid'] . '|' . 0);
            $goods_image_mobile = array();
            if (!empty($image_more)) {
                foreach ($image_more as $val) {
                    $temp_image = cthumb($val['goods_image'], '', $goods_league_info['store_id']);
                    $goods_image_mobile[] = $temp_image . '-ptbw480wm';
                }
            } else {
                $goods_image_mobile[] = cthumb($goods_info['goods_image'], '', $goods_league_info['store_id']) . '-ptbw480wm';
            }*/
            $out_data['good_info'] = $goods_info;
            $out_data['good_image'] = $goods_image_mobile;
//            $this->adt_wGoodsCacheSort($goods_id,$store_id,$out_data);
//        }
        return $out_data;
    }
     ...
     * 产品库商品详情
     */
    public function adtGoodsDetailSource($goods_id){
        $field[]='goods_id';
        ...
        $field[]='goods_size';
        $field[]='validity_time';
        $goods_info=$this->getGoodsInfoByID($goods_id,implode(',',$field));
        if (!$goods_info){
            return array();
        }
        $goods_common_info = Model('goods_common')->field(array('mobile_body','brand_name'))->where(array('goods_commonid' => $goods_info['goods_commonid']))->find();
        if (!$goods_common_info){
            return array();
        }
        $goods_info['goods_image'] = cthumb($goods_info['goods_image'], '', $goods_info['store_id']) . '-ptbw480wm';
        // 解析手机端商品详情信息
        $goods_info['mobile_body'] = $this->mobile_bodySeriToHtml($goods_common_info['mobile_body'], true);
        $goods_info['brand_name']=$goods_common_info['brand_name'];
        return $goods_info;
    ...
    /*
     * 商品手机端详情，序列变化字符转成html
     */
    public function mobile_bodySeriToHtml($serialized_mobile_body,$is_adt=false){
        $water_pirnt=($is_adt)?'-ptbw480wm':'-w480wm';
        $mobile_body = '';
        $mobile_body_array = mb_unserialize($serialized_mobile_body);
        if (is_array($mobile_body_array)) {
            foreach ($mobile_body_array as $val) {
                switch ($val['type']) {
                    case 'text':
                        $mobile_body .= '<pre>' . $val['value'] . '</pre>';
                        break;
                    case 'image':
                        //七牛云压缩图片处理
                        $image_url=$val['value'];
                        ...
                        if($tail!==false){
                            $image_url=str_ireplace($tail,'',$image_url);
                            $image_url=$image_url.$water_pirnt;
                        }else{
                            $image_url=$image_url.$water_pirnt;
                        }
                        if('android'==strtolower($_REQUEST['client_type'])||'ios'==strtolower($_REQUEST['client_type'])){
    ...
    /**
     * 跑腿邦v1.1，获取商品列表，完整的数据
     */
    public function adtGetGoodsListComplete($store_id,$class_id=0,$goods_name='',$random=false){
        $condition['league_goods_verify']=1;
        $class_id && $condition['gc_id_1']=intval($class_id);
        $goods_name && $condition['goods_name']=array('like','%'.$goods_name.'%');
        ...
        $field[]='gc_id_1';
        if($random){
            $condition=array('league_goods_verify'=>1,'league_store_id'=>intval($store_id));
            $goods_list = $this->adtGetGoodsList($condition, $field,'rand()',6);
        }else {
            $goods_list = $this->adtGetGoodsList($condition, $field);
            $count=Model('goods_league')->where($condition)->count();
