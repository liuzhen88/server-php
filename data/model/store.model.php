...
class storeModel extends Model {
     ...
     * @return array
	 */
    public function getStoreOnlineList($condition, $page = null, $order = '', $field = '*', $limit = '') {
    ...
    public function getStoreInfo($condition) {
        ...
        $store_info['store_credit_old'] = $store_info['store_credit'];
        if(!empty($store_info)) {
    ...
    public function delStore($condition){
        ...
        $QNupload->delFileQny(ATTACH_STORE.DS.$store_info['store_label']);
        $QNupload->delFileQny(ATTACH_STORE.DS.$store_info['store_banner']);
...
//        @unlink(BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.$store_info['store_banner']);
        if($store_info['store_slide'] != ''){
            foreach(explode(',', $store_info['store_slide']) as $val){
                $QNupload->delFileQny(ATTACH_STORE.DS.$val);
//                @unlink(BASE_UPLOAD_PATH.DS.ATTACH_SLIDE.DS.$val);
    ...
    //获取店铺8个销量最高商品
        private function getGoodsListBySales($store_array) {
			$model = Model();
            $model_goods = Model('goods');
                $field = 'goods_id,goods_commonid,store_id,goods_name,goods_image,goods_price,goods_salenum,color_id';
                foreach ($store_array as $value) {
                        ...
                        $commonid = array(); // 商品公共id数组
                        $storeid = array();  // 店铺id数组
                        $goodsimage_more= array();
                        
                        if(!empty($store_array[$value['store_id']]['search_list_goods'])){                             
                                foreach ($store_array[$value['store_id']]['search_list_goods'] as $value) {
                                ...
                                foreach ($store_array[$value['store_id']]['search_list_goods'] as $key => $value) {
                                        // 商品多图
                                        foreach ($goodsimage_more as $v) {
                                                if ($value['goods_commonid'] == $v['goods_commonid'] && $value['store_id'] == $v['store_id'] && $value['color_id'] == $v['color_id'] && $v[is_default]=="1") {
                                                        $store_array[$value['store_id']]['search_list_goods'][$key]['goods_image']= $v[goods_image];
                                                }
                                        }
                                }
                        }
     ...
     * 根据经纬度获取查找附近一个店铺
     */
    public function adt_get_store_by_lication($lat,$lng){
        $distance_sql=', round(6378.138*2*asin(sqrt(pow(sin(('.floatval($lat).'*pi()/180-lat*pi()/180)/2),2)+cos('.floatval($lat).'*pi()/180)*cos(lat*pi()/180)* pow(sin( ('.floatval($lng).'*pi()/180-lng*pi()/180)/2),2)))*1000) as distance';
        $field='store_id,store_name,ship_time,open_state';
        $condition['store_type']=4;
        $condition['store_state']=1;
//        $condition['open_state']=1;
//        $condition[$distance_sql_exp']=array('elt',ADT_AVALIABLE_DISTANCE);
        $condition[]=array('exp','lat is not null and lng is not null');
        $store_info=Model('store')->where($condition)->field($field.$distance_sql)->order('distance asc')->find();
        if(!empty($store_info) && $store_info['distance']>ADT_AVALIABLE_DISTANCE){
            $store_info=array();
        }
        return $store_info;
    }
