    ...
    $('a[nc_type="batchbutton"]').click(function(){
        ...
        var handleResult = function(uri,name,type) {
	        ...
	        $('.checkitem:checked').each(function(){
                if(type&&$(".icon-level-up").length>0)
                {
                    var price=parseFloat($(this).parents("tr").next().find(".price_CN").html().replace("¥",''));
                    if(price<=0.0)
                    {
                        showDialog('有价格为零的商品，不能上架');
                        flg=false;
                        return false;
                    }
                }
...
//	        window.location.href=uri + '&' + name + '=' + items;
        ...
        if($(this).attr('confirm')){
        	showDialog($(this).attr('confirm'), 'confirm', '', function(){handleResult(_uri,_name,0)});
        	return false;
        }
		handleResult(_uri,_name,1);
    });
