...
function load_history_information() {
    ...
    t.fn.nc_region = function(a) {
        ...
        function d(a, n) {
            ...
            //扩展了show_grade来控制联动显示层级
            var status = true;
            if ('undefined' != typeof(e.attr('show_grade')) ) {
                var log =  e.attr('show_grade');
                var a_arr = e.val().split(" ");
                status =  a_arr.length < log ? true : false;
                if (!status) {
    ...
    t.fn.nc_region_check = function(a) {
        var e = t(this);
        var n = t.extend({},
        {
            area_id: 0,
        ...
        return this.each(function() {
            ...
            if (a.val() === "") {
                ...
                var area_arr = a.val().split('|');
                var e = t('<span class="' + n.region_span_class + '">' + area_arr[1] + "</span>");
                var i = t('<i class="icon-edit" style="margin-right:10px;"></i>');
                a.after(e);
                e.before(i);
                i.on("click",
                function() {
        ...
        function d(a, n) {
            if (a && nc_a[n].length > 0) {
                ...
                a.append("<option>-请选择-</option>");
                for (i = 0; i < o.length; i++) {
                    a.append("<option value='"+ o[i][0] +"|" + o[i][1] + "'>" + o[i][1] + "</option>")
                }
            }
            
           a.on("change",  function() {
                t(this).nextAll(":checkbox,span").remove();
                $(this).prev().val(a.val());
                var area = a.val().split('|');
                ...
                var o = [];
                o = nc_a[n];
                var output = "<input style='margin-left:10px; vertical-align:middle;' type='checkbox' value='" + p + "' id='area_all_" + n + "' name='area_all[" + n + "]' checked='checked'><span>全部</span>";
                for (i = 0; i < o.length; i++) {
                   output += "<span style='display:inline-block;'><input style='margin-left:10px; vertical-align:middle;' type='checkbox' value='" + o[i][1] + "' name='area[" + n + "][" + o[i][0] + "]' checked='checked'>" + o[i][1] + "</span>";
                }
                t(this).after(output);
                
                // 全部 全选/取消全选
                var area_all_obj = a.nextAll(":checkbox[id='area_all_" +  n + "']");
                area_all_obj.on('change', function() {
                    if (area_all_obj.attr('checked')) {
                        $(this).parent().find('span :checkbox').attr('checked', true);
                    } else {
                        $(this).parent().find('span :checkbox').attr('checked', false); 
                    }
                });
            }
          });
    ...
    t.fn.nc_class_check = function(a) {
        var e = t(this);
        var n = t.extend({},
        {
            gc_id: 0,
            gc_span_class: "_gc_value"
        },
        a);
        ...
        return this.each(function() {
            ...
            if (a.val() === "") {
                ...
                var e = t('<span class="' + n.gc_span_class + '">' + a.val() + "</span>");
                var i = t('<input type="button" value="编辑" />');
                a.after(e);
                e.after(i);
                i.on("click",
        ...
        function d(a, n) {
            ...
            if (a && nc_b.length > 0) {
                ...
                a.append("<option>-请选择-</option>");
                for (i = 0; i < o.length; i++) {
                    a.append("<option data-explain='" + o[i].commis_rate + "' value='" + o[i].gc_id + "|" + o[i].gc_name + "'>" + o[i].gc_name + "</option>");
                }
            }
           ...
           a.on("change",  function() {
                ...
                $(this).prev().val(a.val());
                var myclass = a.val().split('|');
                
                n = myclass[0];
                p = myclass[1];
   
                t.getJSON(SITEURL + '/index.php?act=index&op=josn_class&callback=?', {'gc_id': n}, function (o) {
                        if (o.length > 0) {
                            var output = "<input style='margin-left:10px; vertical-align:middle;' type='checkbox' value='" + p + "' id='class_all_" + n + "' name='class_all[" + n + "]' checked='checked'><span>全部</span>";
                            for (i = 0; i < o.length; i++) {
                               output += "<span style='display:inline-block;'><input style='margin-left:10px; vertical-align:middle;' type='checkbox' value='" + o[i].gc_name + "' name='class[" + n + "][" + o[i].gc_id + "]' checked='checked'>" + o[i].gc_name + "</span>";
                            }
                            a.after(output);
               
                            // 全部 全选/取消全选
                            var area_all_obj = a.nextAll(":checkbox[id='class_all_" + n + "']");
                            area_all_obj.on('change', function() {
                                if (area_all_obj.attr('checked')) {
                                    $(this).parent().find('span :checkbox').attr('checked', true);
                                } else {
        ...
        function l(a) {
            if (typeof nc_b === "undefined") {
                t.getJSON(SITEURL + '/index.php?act=index&op=josn_class&callback=?', {'gc_id':n.gc_id},
                function(t) {
                    nc_b = t;
                    a()
                })
