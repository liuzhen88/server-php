...
class paymentLogic {
    ...
    public function getRealOrderInfo($pay_sn, $member_id = null) {
        ...
        $condition = array();
        $condition['pay_sn'] = $pay_sn;
        $order_list = $model_order->getNormalOrderList($condition, '', '*', 'order_id desc', '', array('order_goods'));
        //新增商品数量用于在支付时显示提示信息
        $buy_goods_num = 0;
        //计算本次需要在线支付的订单总金额
        $pay_amount = 0;
        if (!empty($order_list)) {
            foreach ($order_list as $order_info) {
                if (isset($order_info['extend_order_goods'])){
                    foreach ($order_info['extend_order_goods'] as $key_g=>$row_g){
                        $buy_goods_num += $row_g['goods_num'];
                    }
                }
        ...
        $order_pay_info['buy_goods_num'] = $buy_goods_num;
    ...
    /**
     * 取得所使用支付方式信息,后台退款所用,不判断支付文件所用
     * @param string $payment_code
     */
    public function getPaymentInfo2($payment_code) {
        if (in_array($payment_code,array('offline','predeposit')) || empty($payment_code)) {
            return callback(false,'系统不支持选定的支付方式');
        }
        $model_payment = Model('payment');
    ...
    {
        $config = $this->getPaymentInfo2('alipay');
        if(!$config['state']){
            return false;
        }

        $alipay_config['account'] = $config['data']['payment_config']['alipay_account'];
        $alipay_config['key'] = $config['data']['payment_config']['alipay_key'];
        $alipay_config['partner'] = $config['data']['payment_config']['alipay_partner'];
        $alipay_config['input_charset'] = 'utf-8';
        $alipay_config['sign_type'] = 'MD5';
        $alipay_config['refund_notify_url'] = urlAdmin('api','refundAlipayNotify');
