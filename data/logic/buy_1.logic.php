    ...
    public function calcCartList($store_cart_list) {
        ...
        foreach ($store_cart_list as $store_id => $store_cart) {
            ...
            foreach ($store_cart as $key => $cart_info) {
                ...
                $store_cart[$key]['goods_image_url'] = cthumb($store_cart[$key]['goods_image'],60);
        ...
        foreach ($store_goods_total as $store_id => $goods_total) {
            ...
            if (is_array($rule_info) && !empty($rule_info)) {
                ...
                if (!empty($rule_info['mansong_goods_name']) && !empty($rule_info['goods_storage'])) {
                    ...
                    $data['goods_image_url'] = cthumb($rule_info['goods_image'],60);
...
//         if (empty($pd_payment_info)) return;
        ...
        {
            $available_pd_amount = bcsub($available_pd_amount, $store_pre_deposit['amount_total'], 2);
        }
	...
	/**
	 * 消费码编号(2位随机码 + 7位订单表主键id，确保不会有重复 + 3位当前微秒数)
	 * @param unknown $order_id
	 * @return string
	 */
	public function makeConsumeCode($order_id)
	{
	    return  mt_rand(10,99)
	    . sprintf('%07d',substr($order_id,-7)) 
	    . sprintf('%03d', (float) microtime() * 1000);
	}
    ...
    public function getReciverAddr($address_info = array()) {
        if (intval($address_info['dlyp_id'])) {
            ...
            $reciver_info['door_number'] = isset($address_info['door_number'])?$address_info['door_number']:'';
            $reciver_info['address'] = $address_info['area_info'].' '.$address_info['address'].' '.$reciver_info['door_number'];
            $reciver_info = serialize($reciver_info);
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        $new_array['goods_price'] = del0($goods_info['goods_price']);
        ...
        $new_array['goods_freight'] = del0($goods_info['goods_freight']);
    ...
    private function _getOnlineCartList($cart_list) {
        ...
        foreach ((array)$cart_list as $key => $cart_info) {
            ...
            if (in_array($cart_info['goods_id'],array_keys($goods_online_array))) {
                ...
                $cart_list[$key]['goods_price'] = del0($goods_online_info['goods_price']);
                $cart_list[$key]['transport_id'] = $goods_online_info['transport_id'];
                $cart_list[$key]['goods_freight'] = del0($goods_online_info['goods_freight']);
     ...
     * @param $shipping_fee 是否有运费，true有，false免运费
     */
    public function adtGetUseableCoupon($member_id,$order_amount,$shipping_fee=0){
        ...
        $now=time();
        $c_condition['member_id']=$member_id;
        $c_condition['is_used']=0;
        $c_condition[]=array('exp','validity_start_time*is_validity<='.$now);
        $c_condition['validity_end_time']=array('egt',array('exp',$now.'*is_validity'));
        $coupon_list=Model('member_coupon_league')->where($c_condition)->order('id desc')->select();
        ...
        foreach($coupon_list as $value){
            //1.过滤不能用的优惠券
            //运费已经为0的不使用免运费优惠券
            if(0==$shipping_fee && 1==$value['coupon_type']){
                continue;
            }
            //排除不能使用的满减券
            if(2==$value['coupon_type'] && $value['coupon_limit_amount']>$goods_amount){
                continue;
            }
            ...
            //不论第一个是免运费还是满减优惠券，后面的免运费都不会比第一个更实惠
            if(1==$value['coupon_type']){
                continue ;
            }
            if(1==$best['coupon_type'] && ADT_CARRIAGE>$value['coupon_amount']){
                continue ;
            }
            if(2==$best['coupon_type'] && $best['coupon_amount']>$value['coupon_amount']){
                continue ;
            }
            $best=$value;
        }
