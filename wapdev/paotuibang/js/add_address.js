$(function(){
    ...
    $("#details").on('click',function(){
        window.location.href = WapSiteUrl + "/aidatui/map.html?name="+$("#name").val()+"&mob="+$("#mob").val()+"&addrNum="+$("#detailsNum").val()+"&lat="+lat+"&lng="+lng+"&status=add&order="+order;
    });
            ...
            if(isPhone()&&isName()&&isAddress()){
                $.ajax({
                    ...
                    success: function (data) {
                        if(data.code==200){
                            if(order=='yes'){
                                window.location.href = WapSiteUrl + "/aidatui/confirm_order.html?address_id="+data.data.address_id;
                            }else{
