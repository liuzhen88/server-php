...
class member_cartControl extends mobileMemberControl {
    ...
    public function cart_listOp() {
        ...
        foreach ($cart_list as $key => $value) {
            ...
            $cart_list[$key]['goods_price'] = del0($value['goods_price']);
            $cart_list[$key]['goods_sum'] = del0($cart_list[$key]['goods_sum']);
    ...
    /**
     * functionname   : 新获取购物车列表
     * author         : xuping
     */
    public function new_cart_listOp() {
        $model_cart = Model('cart');
        $condition = array('buyer_id' => $this->member_info['member_id']);
        $cart_list	= $model_cart->new_listCart('db', $condition);
    ...
    public function cart_addOp() {
        ...
        //分销相关功能
        $store_id = isset($_REQUEST['dis_store_id'])?intval($_REQUEST['dis_store_id']):0;               //分销店铺id
        $member_id = isset($_REQUEST['dis_member_id'])?intval($_REQUEST['dis_member_id']):0;            //分销用户id
        $param=Logic('distribution')->get_cart_data($goods_info['goods_id'],$store_id,$member_id);
...
//        $cart_id = intval($_REQUEST['cart_id']);
        $cart_id_array=array();
        $cart_id_array=explode(',',$_REQUEST['cart_id']);
        $model_cart = Model('cart');
        if(is_array($cart_id_array)) {
            $condition = array();
