...
class articleControl extends mobileHomeControl{
    ...
    public function article_listOp() {
        if(!empty($_REQUEST['ac_id']) && intval($_REQUEST['ac_id']) > 0) {
			$article_class_model	= Model('article_class');
			$article_model	= Model('article');
			$condition	= array();
			
			$child_class_list = $article_class_model->getChildClass(intval($_REQUEST['ac_id']));
			$ac_ids	= array();
    ...
    private function article_type_name() {
    	if(!empty($_REQUEST['ac_id']) && intval($_REQUEST['ac_id']) > 0) {
			$article_class_model = Model('article_class');
			$article_class = $article_class_model->getOneClass(intval($_REQUEST['ac_id']));
    ...
    public function article_showOp() {
        ...
        if(!empty($_REQUEST['article_id']) && intval($_REQUEST['article_id']) > 0) {
			$article	= $article_model->getOneArticle(intval($_REQUEST['article_id']));
