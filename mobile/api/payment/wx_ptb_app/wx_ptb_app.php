...
class wx_ptb_app{
    ...
    public function __construct() {
       ...
       // $this->appid = 'wx38e270b661319a2c';
        $this->appid =  $this->payment['payment_config']['wxpay_appid'];
       // $this->mch_id = '1267913901';
        $this->mch_id = $this->payment['payment_config']['wxpay_mch_id'];

       // $this->key = 'wx38e270b661319a2cwx38e270b66131';
        $this->key = $this->payment['payment_config']['wxpay_key'];
        // $this->redirect_uri = 'http://'.$_SERVER['SERVER_NAME'].$this->redirect_uri;

       // $this->redirect_uri = 'http://51aigegou.cn/aigegou/test.html';
        $this->notify_url = MOBILE_SITE_URL.'/api/payment/wx_ptb_app/notify_url.php';
    ...
    public function submit($orderparam = array()){
        //print_r($orderparam);die;
        
        //       '&'.$this->redirect_uri.
        //       '&'.'response_type=code';

        $this->dowxpay($orderparam['order_type'].$orderparam['order_sn']);
        exit;
    ...
    private function dowxpay( $order_sn){
        $order_body = '商品支付';
        ...
        if($order_type == 'r'){
            ...
            if (isset($result['data']['order_list']))
            {
                ...
                {
                    $order_body = $row['store_name'];
                    if (isset($row['extend_order_goods']))
                    {
                        foreach ($row['extend_order_goods'] as $key_goods=>$row_goods)
                        {
                            $order_body = $row_goods['goods_name'];
                        }
                    }
                    break;
                }
            }
            $order_body=empty($order_body) ? '商品支付' : $order_body;
        }elseif($order_type == 'v'){
...
//         $this->parameters['body']=isset($order_body) ?  $order_body : '商品买单' ;
        $this->parameters['body']=$order_body;
        //file_put_contents('body.log', $order_body);
        ...
        function jsApiCall()
        {
            WeixinJSBridge.invoke(
                ...
                function(res){
                    ...
                    window.location.href="{$wap_site_url}/tmpl/member/order_list.html"; 
...
eot;
    ...
    public function getParameters()
    {
        ...
        $jsApiObj['partnerid'] = $this->mch_id;
