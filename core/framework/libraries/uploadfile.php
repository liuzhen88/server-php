...
class UploadFile{
	 ...
	 * 允许上传的文件扩展名类型
	 */
	private $allow_type=array('gif','jpg','jpeg','bmp','png','swf','tbi','mp3');
        
        /**
         * 允许上传的文件类型 by lijunhua 2015-08-14
         */ 
        private $allow_file_type = array('image/jpeg', 'image/pjpeg', 'image/gif',  'image/x-png', 'image/png', 'application/octet-stream', 'image/bmp');
	...
	function __construct(){
		...
		Language::read('core_lang_index');
                ...
                $this->max_size = C('image_max_filesize') ? C('image_max_filesize') : $this->max_size;
                $this->allow_type = C('image_allow_ext') ? explode(',', C('image_allow_ext')) : $this->allow_type;
	}
	/**
	 * 设置
	 *
	...
	public function upfile($field){
		...
		//验证文件合法性
	    if(!in_array($this->upload_file['type'], $this->allow_file_type)){
	        $error = '文件类型不合法';
	        $this->setError($error);
	        return false;
	    }
	    ...
	    {
	        $old_file_path = rtrim($this->save_path, DS).DS.$this->file_name;
	        $this->delFileQny($old_file_path);
	    }  
	    ...
	    $file_path = rtrim($this->save_path, DS).DS.$this->file_name;
	    $result = $this->uploadFileQny($this->upload_file['tmp_name'], $file_path);
	 ...
	 * )
	 */
	function upBase64Image($file_string)
	{
	    if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $file_string, $result)){

	        $this->ext  = $result[2];
	        $file_string = base64_decode(str_replace($result[1], '', $file_string));
	        if(version_compare(PHP_VERSION,'5.4.0','>=') && !$image_info = @getimagesizefromstring($file_string)) {
	            $error = Language::get('upload_image_is_not_image');
	            $this->setError($error);
	            return false;
	        } 
	        ...
	        {
	            $this->file_size = strlen($file_string);
	            ...
	            $file_path = rtrim($this->save_path, DS).DS.$this->file_name;
	            $result = $this->uploadStringImgQny($file_string, $file_path);
	            $this->thumb_image = $this->file_name;
	            return $result;
	        }    
	    }
	 ...
	 * @return boolean
	 */
	function uploadFileQny($filepath, $key )
	{
	    ...
	    if (empty($accessKey))
	    {
	        if (isset($GLOBALS['config']['qny']) && !empty($GLOBALS['config']['qny']))
	        {
	            $accessKey = $GLOBALS['config']['qny']['accessKey'];
	            $secretKey = $GLOBALS['config']['qny']['secretKey'];
	            $bucket = $GLOBALS['config']['qny']['bucket'];
	        }
	        else 
	        {
	            $this->setError('七牛云参数错误');
	            return false;
	        }
	    ...
	    //$filePath = BASE_DATA_PATH.'/api/qiniuyun/examples/php-logo.png';
	    //$key = 'php-logo.png';
	 ...
	 * @return boolean
	 */
	function uploadStringImgQny($data, $key )
	{
	    require_once BASE_DATA_PATH.'/api/qiniuyun/src/Qiniu/functions.php';
	    //use Qiniu\Auth;
	    //use Qiniu\Storage\UploadManager;
	 ...
	 * @return bool
	 */
	public function upfile_old($field){
