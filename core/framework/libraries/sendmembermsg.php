...
class sendMemberMsg {
    ...
    public function send($param = array()) {
        ...
        if (empty($setting_info) || $setting_info['is_receive']) {
            ...
            if ($tpl_info['mmt_message_switch']) {
                ...
                $message_title = isset($param['message_title']) ? $param['message_title'] : '';
                $this->sendMessage($message, $message_title);
            }
            // 发送短消息
     ...
     */
    private function sendMessage($message, $message_title = '') {
        //添加短消息
    ...
    private function sendShort($number, $message) {
        $sms = new Sms();
        $sms->notice_send($number, $message);
    }
