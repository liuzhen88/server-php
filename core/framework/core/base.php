...
final class Base{
	...
	private static function parse_conf(&$setting_config){
		...
		$setting['shop_version'] = '<span class="vol"><font class="b">A</font><font class="o">igegou</font></span>';
	...
	private static function start_session(){
		...
		@ini_set("session.save_handler", C('session_type'));
	...
	public static function autoload($class){
	    $oldclass = str_replace('\\', '/', $class);
	    //chenyifei
	    $qiniu_pic_path = BASE_DATA_PATH.'/api/qiniuyun/src/';
		...
		//chenyifei
		elseif (is_file($qiniu_pic_path.$oldclass.'.php'))
		{
		    include_once $qiniu_pic_path.$oldclass.'.php';
		}
		else{
	...
	private static function cp(){
		...
		if (strpos(self::CPURL,'||') !== false){
			...
			header('location: http://shopnc.net');exit();
		}else{
