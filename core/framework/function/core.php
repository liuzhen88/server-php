...
//    $file = str_ireplace('.', '_' . $key . '.', C('default_goods_image'));
    ...
    $qnycthumbType = qnycthumbType(C('default_goods_image'), $key);
    $file = ($key == '') ? C('default_goods_image') : $qnycthumbType;
...
function getMemberAvatar($member_avatar){
    if (empty($member_avatar)) {
        ...
        if (substr($member_avatar, 0 ,11) == 'aigegouold_')
        {
            $old_file = str_replace('aigegouold_', '', $member_avatar);
            return UPLOAD_SITE_URL.DS.$old_file;
        }
...
//	if(check_remote_file_exists(UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.'/avatar_'.$id.'.jpg')){
...
function getMemberAvatarForID($member_id) {
    ...
    $member_info = $model_member->getMemberInfoByID($member_id, 'member_avatar');
    //给默认头像
    if (empty($member_info) || empty($member_info['member_avatar'])) {
       $member_avatar =  UPLOAD_SITE_URL . '/' . ATTACH_COMMON . DS . C('default_user_portrait');
    }
	$member_avatar=getMemberAvatar($member_info['member_avatar']);

	//这种逻辑会导致系统响应非常慢。lixiyu
//    $member_avatar = UPLOAD_SITE_URL . '/' . ATTACH_AVATAR . '/' . $member_info['member_avatar'];
//    if (!check_remote_file_exists($member_avatar)) {
//        if (check_remote_file_exists(UPLOAD_SITE_URL.DS.$member_info['member_avatar']))
//        {
//            //qnymove($member_info['member_avatar'], ATTACH_AVATAR.DS.$member_info['member_avatar']);
//            return UPLOAD_SITE_URL.DS.$member_info['member_avatar'];
//        }
//       $member_avatar =  UPLOAD_SITE_URL . '/' . ATTACH_COMMON . DS . C('default_user_portrait');
//    }
 ...
 * 取得分类图标
 */
function getQiniuyunimg($path, $img)
{
    $basepath = UPLOAD_SITE_URL.DS;
    $filepath = UPLOAD_SITE_URL.DS . $path .DS . $img;
    if (empty($img))
    {
        return '';
    }
 ...
 * @param string $type 查询类型 store_logo/store_avatar/league_store_avatar(跑腿邦商户默认头像)
 * @return string
 */
function getStoreLogo($img, $type = 'store_avatar'){
    if ($type == 'store_avatar') {
        if (empty($img)) {
            ...
            if (substr($img, 0 ,11) == 'aigegouold_') 
            {
                return UPLOAD_SITE_URL.DS.str_replace('aigegouold_', '', $img);
            }
        ...
        if (empty($img)) {
            ...
            if (substr($img, 0 ,11) == 'aigegouold_') 
            {
                //qnymove($file, ATTACH_GOODS . '/' . $store_id . '/' .$file);
                return  UPLOAD_SITE_URL.DS.str_replace('aigegouold_', '', $img);
            }
            return UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.$img;
        }
    }elseif ($type == 'league_store_avatar' ){
        if (empty($img)) {
            return UPLOAD_SITE_URL.DS.ATTACH_COMMON.DS.C('default_league_store_avatar');
        } else {
            return UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.$img;
        }
    }
...
function getStoreDecorationImageUrl($image_name = '', $store_id = null) {
    ...
    if(!empty($image_name) && !empty($store_id)) {
...
function write_file($filepath, $data, $mode = null)
{
    ...
    $data = "<?php defined('emall') or exit('Access Invalid!'); return ".$data.";";
...
function agg_array_column($array,$column){
	$return=array();
...
/**
 * 重新组织md5加密(加密次数控制在2-4次)
 * 
 * @param string $str     密码
 * @param string $salt    随机码
 * @param string $pre_str 前缀字符串
 */
function md6($str, $salt = '', $pre_str = 'agg_') {
    return md5($pre_str . md5($str) . $salt);
}
 ...
 * @author lijunhua
 * @since  2015-08-19
 * @param int    $length  字符串度
 * @param boolen $lower   是否大小写
 * @return string 
 */
function random_str($length, $lower = true) {
    $return_str = '';
    $pattern = 'ab1CD2ef3GH4ijk5LMN6gp7qrs8TUVW9sy0z';
    for ($i = 0; $i < $length; $i ++) {
        $return_str .= $pattern {mt_rand(0, 35)}; //生成php随机数
    }
    return $lower ? strtolower($return_str) : $return_str;
}
 ...
 * @param Array $extend 其他参数 'client_type'=>array("android", "ios", "winphone") 客户端类型，extras 传递给客户端的参数 'extras'=>array('hello'=>'word',9)
 * @return 推送成功，true。推送失败false(1.消息超过限制，2.极光返回失败,3.指定会员都没有设备id,4.全部推送失败)
 */
/* function JPush($message,$member_ids=array(),$extend=array()){
    require_once BASE_ROOT_PATH.DS.DIR_RESOURCE.DS.'vendor/agg_push.php';
    $data=Model('member_common')->selectMemberComonByMemberId($member_ids,'distinct registration_id',array('registration_id'=>array('neq','')));
    //1.推送给所有用户
    if(empty($member_ids)) {
...
/**
 * 成功接口新调用方法(2015/12/14 开始新写的接口开始可用)
 * @param unknown $datas 具体信息值
 * @param string $message 提示信息
 */
function output_data_msg($datas=array(), $message='ok') {
    $data = array(
        'code' => 200,
        'message' => $message,
        'data' => $datas,
...
/**
 * 成功接口信息调用(旧方法，已作废，只用在旧接口和旧接口修改时，2015/12/14 开始新写的接口不可用)
 * @param unknown $datas
 * @param unknown $extend_data
 */
...
/**
 * 检查request参数是否合法
 * @param $rule array 必填的参数，数组用.表示。如$rule=array('data.member_name','data.store_name','city_info');
 */
function check_request($rule){
...
//调式代码执行时间专用方法
function debug_set_start_time(){
	define('XIYU_DEBUG_START_TIME',microtime());
	$str='<pre><font  color="red"> setting start time :'.XIYU_DEBUG_START_TIME.'</font></pre>';
}
function debug_set_end_time(){
	define('XIYU_DEBUG_END_TIME',microtime());
	$str='<pre><font  color="red"> setting end time :'.XIYU_DEBUG_END_TIME.'</font></pre>';
}
function debug_print_excute_time(){
	$start_time=explode(' ',XIYU_DEBUG_START_TIME);
	$end_time=explode(' ',XIYU_DEBUG_END_TIME);
	$params=debug_get_request_str();
	$str='<pre><font  color="blue"> request string is :'.($params).'</font></pre>';
	$str.='<pre><font  color="red"> excute time is :'.($end_time[0]+$end_time[1]-$start_time[0]-$start_time[1]).'</font></pre>';
	file_put_contents('xiyu_debug.html',$str,FILE_APPEND);
}

function debug_get_request_str(){
	$request=$_REQUEST;
	$str='';
