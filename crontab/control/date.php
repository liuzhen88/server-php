...
//         $this->_add_points();
        ...
        //分销金额统计(定时任务，每天一次)
        $this->_distribution_amount_statistics();

        //本土商品过期订单自动退款
        $this->_reset_money();

        //跑腿邦，统计每月送达率
        $this->_statistics_send_percent();
     ...
     * 跑腿邦，统计每月送达率
     */
    private function _statistics_send_percent(){

        $store_list = Model('store')->getStoreOnlineList(array('store_type'=>'4'),null,'','store_id');

        $model_order = Model('order');

        foreach ($store_list as $store_info) {
            $result = $model_order->getOrderSendPercent($store_info['store_id']);
            if(!$result){
                $result = 100;  //默认送达率为100%
            }
            $model_order->execute('replace into agg_adt_store_home_stat set store_id='.$store_info['store_id'].',send_percent='.$result);
        }
    }
     ...
     * @since  2015-08-27
     */
    private function _goods_offline_update() {
        //agg_goods表处理关联到goods_common
        Model('goods')->editProducesOffline(array(
            'goods_state'   => 1, 
            'good_type'     => 1, 
            'validate_time' =>  array('between',array(1, time()))
        ));
    ...
    /**
     * 随机更新线上商品销量
     *  每天上午10点，下午3点各执行一次，随机增量1-100
     */
    public function update_goods_salenumOp()
    {
        Model('goods')->editGoods(array('goods_salenum'=>array('exp', 'goods_salenum + FLOOR((RAND() * 100))')), array('good_type' => 2, 'goods_state' => 1));
    }
    
    /**
     * @time 2015-12-10 19:11 
     * @author solon.ring2011@gmail.com-退款-
     * 过期退款处理
     * @return [type] [description]
     */
    private function _reset_money(){
        //from_unixtime(1218169800, '%Y %D %M %h:%i:%s %x');
        $where          =   "left(from_unixtime(validate_time),10)='".date('Y-m-d',strtotime('-1 days'))."' and validate_time>0 and goods_verify=1 and good_type=1 and goods_state=1";
        $model          =   Model('goods');
        $model_order    =   Model('order');
        $rt             =   $model->getGoodsListId($where);
        foreach ($rt as $key => $value) {
            //$offline    =   $model->editProducesOffline(array('goods_commonid'=>$value['goods_commonid']));
            $g          =   $model_order->getOrderGoodsList(array('goods_id'=>$value['goods_id']));
            if(count($g)>0)
            {
                foreach ($g as $k => $v) 
                {
                    $grt = $model_order->getOrderInfo('order_id='.$v['order_id'].' and order_state in (10,20) and delete_state=0 and refund_state=0 and order_type=1');
                    if($grt['order_state'] == 10)
                    {
                        $data['order_state']    =   0;
                        $data['refund_state']   =   0;
                        $model_order->editOrder($data,array('order_id'=>$v['order_id']),1);
                    }
                    else
                    {
                        $ids['order_id']       =   $v['order_id'];
                        $ids['order_money']    =   $grt['order_amount'];
                        $ids['member_id']      =   $grt['buyer_id'];
                        $ids['member_name']    =   $grt['buyer_name'];
                        $ids['order_sn']       =   $grt['order_sn'];
    ...
    /**
     * 统计前一天数据插入distribution_statis表中。
     * @return [type] [description]_distribution_amount_statistics
     */
    private function _distribution_amount_statistics(){
        $insert_array = array();
        $mds = Model('distribution_statis');
        $records = Model('distribution_records')->getRecordsAll();
        $marray = array();
        foreach ($records as $key => $value) {
            $t = str_replace('-','', $value['ymd']);
            // $marray[$t][$value['dis_member_id']]['member_id'] =0;
            // $marray[$t][$value['dis_member_id']]['store_id'] = 0;
            // $marray[$t][$value['dis_member_id']]['store_id'] = 0;
            // $marray[$t][$value['dis_member_id']]['member_id'] = 0;
            $marray[$t][$value['dis_member_id']]['first_price'] = $value['first_price'];
            $marray[$t][$value['dis_member_id']]['second_price'] = $value['second_price'];
            $marray[$t][$value['dis_member_id']]['di_totals'] += $value['distribution_money'];
            $marray[$t][$value['dis_member_id']]['distribution_money'] = $value['distribution_money'];
            $marray[$t][$value['dis_member_id']]['distribution_type'] = $value['distribution_type'];
            if($value['distribution_type'] == 4)
                {
                   $marray[$t][$value['dis_member_id']]['di_other_totals'] += $value['distribution_money'];
                   $marray[$t][$value['dis_member_id']]['store_id'] =   $value['dis_store_id'];
                }
                else
                {
                    $marray[$t][$value['dis_member_id']]['di_self_totals'] += $value['distribution_money'];
                    $marray[$t][$value['dis_member_id']]['member_id'] = $value['dis_member_id'];
                }
            $marray[$t][$value['dis_member_id']]['ymd'] = str_replace('-','', $value['ymd']);
            $marray[$t][$value['dis_member_id']]['ym'] = str_replace('-','', $value['ym']);
            $marray[$t][$value['dis_member_id']]['y'] = $value['y']; 
        }

        $iarray = array(); 
        foreach ($marray as $key => $value) { 
            foreach ($marray[$key] as $k => $v) {
                $iarray['di_day'] = $v['ymd'];
                $iarray['di_month'] = $v['ym'];
                $iarray['di_year'] = $v['y'];
                $iarray['di_totals'] =  $v['di_totals'];
                $iarray['di_other_totals'] = $v['di_other_totals'];
                $iarray['store_id'] =   $v['store_id'];
                $iarray['di_self_totals'] = $v['di_self_totals'];
                $iarray['member_id'] = $v['member_id'];
    

                $iarray['di_create_date'] = time();
                $insert_array[] = $iarray;
            };
            
            //Model('distribution_statis')->addStatis($iarray);
        }
        //print_r($insert_array);
        $mds->insertAll($insert_array);
    }
    /**
     *  查询用户所属区域
     */
    function _set_member_district(){
        //获取未定位区域用户
        $member_sum = Model()->table('member,member_common')->join('inner')->on('member.member_id=member_common.member_id')->where("member_common.lat is not null and member_common.lng is not null and member.member_areaid is null")->count();
        $member_list= Model()->table('member,member_common')->join('inner')->on('member.member_id=member_common.member_id')->field('member_common.*')->where("member_common.lat is not null and member_common.lng is not null and member.member_areaid is null")->limit('0,1000')->select();
        for ($i=1;$i<ceil($member_sum/1000);$i++) {
            $begin = $i*1000;
            $tempList = Model()->table('member,member_common')->join('inner')->on('member.member_id=member_common.member_id')->field('member_common.*')->where("member_common.lat is not null and member_common.lng is not null and member.member_areaid is null")->limit("$begin,1000")->select();
        ...
        foreach($member_list as $item){
            ...
            $ch = curl_init();
            //设置选项，包括URL
            $url = "http://api.map.baidu.com/geocoder/v2/?ak=uLhko8NKBiREseUxAWE0hVYc&callback=renderReverse&location=".$item['lat'].",".$item['lng']."&output=json&pois=0";
            print_r($url);
            ...
            $infoArray = json_decode($output,true);
            if($infoArray['status']==0){
                if(!empty($infoArray['result']['addressComponent']['district'])){
                    //获取省市区id
                    $district_where = array();
                    $district_where['area_name'] = $infoArray['result']['addressComponent']['district'];
                    $district_where['area_deep'] = 3;
                    $district_info = Model('area')->getAreaInfo($district_where);
                    if(!empty($district_info)){
                        $member_update = array();
                        $member_update['member_areaid'] = $district_info['area_id'];//区域id
                        $member_update['member_cityid'] = $district_info['area_parent_id'];//市id
                        //获取省id
                        $province_where = array();
                        $province_where['area_id'] = $district_info['area_parent_id'];
                        $province_info = Model('area')->getAreaInfo($province_where);
                        $member_update['member_provinceid'] = $province_info['area_parent_id'];//省id
                        $member_where['member_id'] = $item['member_id'];
                        //写入区域到数据库
