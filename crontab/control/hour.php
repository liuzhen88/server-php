...
class hourControl extends BaseCronControl {
    ...
    public function indexOp() {
        ...
        //线上产品返利
        $this->_store_online_rebate();

        //跑腿邦，未付款订单超期自动关闭
        $this->_ptb_order_timeout_cancel();
    }

    /**
     * 跑腿邦，未付款订单超期自动关闭
     */
    private function _ptb_order_timeout_cancel(){
        ...
        $condition['add_time'] = array('lt', TIMESTAMP - PTB_ORDER_AUTO_CANCEL_HOUR * self::EXE_TIMES);
        ...
        for ($i = 0; $i < 500; $i++) {
            ...
            foreach ($order_list as $order_info) {
                ...
                if (!$result['state']) {
                    $this->log('跑腿邦 订单超期未支付关闭失败SN:' . $order_info['order_sn']);
                    $_break = true;
                    break;
                }
            }
     ...
     * @since  2015-08-31
     */
    private function _store_online_rebate()
    {
        $condition = array();
        ...
        //用户用户确认订单时大于7天，则实时返利
        $condition['delay_time']  = array('lt',TIMESTAMP - ORDER_RECEIVE_REBATE_DAY * 86400);
        ...
        for ($i = 0; $i < 500; $i++) {
                ...
                $order_list = $model_order->getOrderList($condition, '', '*', 'delay_time asc', 100, array(), true); //查主表处理
                if (empty($order_list)) {
                    break;
                }
                ...
                foreach ($order_list as $order_info) {
                    if ($order_info['refund_state'] != 0) {
                        $refund_count = Model('refund_return')->getRefundReturnCount(array('order_id'=>$order_info['order_id'], 'refund_state' != 3));
                        if ($refund_count > 0) {
                            // 正在退货的订单跳过
                            continue;
                        }
                    ...
                    try {
                        ...
                        $logic_order->setRebate($order_info);
                        $model_order->commit();
                    } catch (Exception $e) {
